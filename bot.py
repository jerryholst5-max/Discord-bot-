...
async def update_partner_panel(guild: discord.Guild):
    ...
    if not partners:
        ...
        embed.set_footer(text="GermanyRP • 0 Partner")
        try:
            if message_id:
    ...
    # Partner nach Kategorie gruppieren
    kategorien = {}
    for p in partners:
    ...
    for kat_name, kat_partners in kategorien.items():
        ...
        for i, p in enumerate(kat_partners, 1):
            ...
            lines.append(f"{i}. **{p['name']}**{ansprechpartner}\n📝 {p['description']}")
        embed.add_field(
            name=f"── {kat_name} ──",
            value="\n\n".join(lines),
            inline=False
...
@app_commands.describe(
    ...
    kategorie="Kategorie z.B. Große Partner, Partner (optional)"
)
async def partner_hinzufuegen(interaction: discord.Interaction, name: str, beschreibung: str, link: str, ansprechpartner: str = None, kategorie: str = "Partner"):
    if not interaction.user.guild_permissions.administrator:
        await interaction.response.send_message("❌ Keine Berechtigung!", ephemeral=True)
        return
    await interaction.response.defer(ephemeral=True)
