...
class SetupSession:
    def __init__(self, guild: discord.Guild, spec_key: str, invoker_id: int):
        self.guild = guild
        self.spec_key = spec_key
        self.spec = SETUP_SPECS[spec_key]
...
class PanelCategorySelect(discord.ui.Select):
    def __init__(self, invoker_id: int, guild: discord.Guild):
        self.invoker_id = invoker_id
        self.guild = guild
        options = [discord.SelectOption(label=spec["label"][:100], value=key) for key, spec in SETUP_SPECS.items()]
        super().__init__(placeholder="Setup-System auswählen...", options=options, min_values=1, max_values=1)

    async def callback(self, interaction: discord.Interaction):
        if interaction.user.id != self.invoker_id:
            await interaction.response.send_message("❌ Dieses Panel ist nicht für dich.", ephemeral=True)
            return
        session = SetupSession(self.guild, self.values[0], self.invoker_id)
...
class PanelCategorySelectView(discord.ui.View):
    def __init__(self, invoker_id: int, guild: discord.Guild):
        ...
        self.invoker_id = invoker_id
        self.guild = guild
        self.add_item(PanelCategorySelect(invoker_id, guild))
...
class PanelGuildSelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        view = PanelCategorySelectView(self.invoker_id, guild)
        try:
