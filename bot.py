...
def load_warnings():
    db = get_db()
    doc = db["warnings"].find_one({"_id": "warnings"})
    return doc.get("data", {}) if doc else {}

def save_warnings(data):
    db = get_db()
    db["warnings"].update_one({"_id": "warnings"}, {"$set": {"data": data}}, upsert=True)

def load_config():
    db = get_db()
    doc = db["config"].find_one({"_id": "config"})
    return doc.get("data", {"alert_users": []}) if doc else {"alert_users": []}

def save_config(data):
    db = get_db()
    db["config"].update_one({"_id": "config"}, {"$set": {"data": data}}, upsert=True)

def load_tickets():
    db = get_db()
    doc = db["tickets"].find_one({"_id": "tickets"})
    return doc.get("data", {}) if doc else {}

def save_tickets(data):
    db = get_db()
    db["tickets"].update_one({"_id": "tickets"}, {"$set": {"data": data}}, upsert=True)
...
@tree.command(name="ticket-kategorie-hinzufügen", description="Fügt eine neue Kategorie zum Ticket-Panel hinzu")
@app_commands.describe(
    kanal="Der Kanal wo das Panel ist",
...
async def ticket_kategorie_hinzufuegen(
    ...
    key = "custom_" + name.lower().replace(" ", "_").replace("-", "_")[:20]

    # Add to TICKET_CATEGORIES
    TICKET_CATEGORIES[key] = (emoji, name, beschreibung)

    # Rebuild and resend panel
    cfg = get_ticket_config(interaction.guild.id)
    if not cfg:
        await interaction.followup.send("❌ Ticket-System ist noch nicht eingerichtet!", ephemeral=True)
        return
    ...
    for k, (e, l, d) in TICKET_CATEGORIES.items():
        embed.add_field(name=f"{e} {l}", value=d, inline=True)
    ...
    await interaction.followup.send(
        embed=liquid_glass_embed(
            ...
            f"**{emoji} {name}** wurde zum Panel hinzugefügt.\nDas Panel wurde in {kanal.mention} neu gepostet.",
            discord.Color.from_rgb(100, 220, 150)
        ),
        ephemeral=True
    )
