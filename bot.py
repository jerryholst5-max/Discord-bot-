...
# Standard-Ticket-Kategorien: key -> (emoji, label, beschreibung)
# HINWEIS: Diese Konstante fehlte komplett im Code (NameError bei /ticket-kategorie-hinzufuegen
# und /ticket-kategorien). Werte ggf. an die tatsächlich gewünschten Standard-Kategorien anpassen.
TICKET_CATEGORIES = {
    "support": ("🎫", "Support", "Allgemeine Fragen und Hilfe"),
    "bug": ("🐛", "Bug-Report", "Fehler oder Probleme melden"),
    "beschwerde": ("⚠️", "Beschwerde", "Beschwerde über einen User oder Vorfall"),
    "sonstiges": ("📁", "Sonstiges", "Alles was nicht in die anderen Kategorien passt"),
}
...
# ─────────────────────────────────────────────
# Interaction-Dedupe (Schutz gegen doppelte Interaction-Events,
# z.B. nach einem Gateway-Resume, inkl. Race Conditions).
# WICHTIG: Diese Funktion darf KEIN await enthalten, damit die
# Prüfung+Markierung atomar bleibt (kein Task-Switch dazwischen).
# ─────────────────────────────────────────────
_recent_interaction_ids: set[int] = set()

def _dedupe_interaction(interaction: discord.Interaction) -> bool:
    """Gibt True zurück, wenn diese Interaction bereits verarbeitet wird/wurde."""
    iid = interaction.id
    if iid in _recent_interaction_ids:
        return True
    _recent_interaction_ids.add(iid)
    if len(_recent_interaction_ids) > 3000:
        _recent_interaction_ids.clear()
    return False

async def safe_interaction_response(interaction: discord.Interaction, **kwargs):
    """Antwortet auf eine Interaction, ohne bei Doppel-Dispatch zu crashen.
    Das passiert z.B. bei Railway-Redeploys, wenn kurzzeitig zwei Bot-Prozesse
    parallel laufen und beide dieselbe Interaction verarbeiten, oder wenn
    Discord nach einem Gateway-Resume ein Event erneut ausliefert.
    followup.send() funktioniert immer, unabhängig davon, welcher Prozess
    die Interaction zuerst per response.send_message()/defer() bestätigt hat,
    da das Webhook-Token 15 Minuten gültig bleibt."""
    try:
        ...
        if getattr(e, "code", None) == 40060 or "already been acknowledged" in str(e):
            try:
                ...
                print(f"[DEDUPE] Interaction-Antwort verworfen (Doppel-Dispatch): {e2}")
        else:
            raise

async def safe_interaction_defer(interaction: discord.Interaction, **kwargs) -> bool:
    """Wie safe_interaction_response, aber für defer(). Gibt zurück, ob defer
    tatsächlich ausgeführt wurde (False = war schon bestätigt, followup geht trotzdem)."""
    try:
...
async def safe_interaction_edit(interaction: discord.Interaction, **kwargs):
    """Wie safe_interaction_response, aber für edit_message()."""
    try:
        if interaction.response.is_done():
        ...
        # Interaction abgelaufen (z.B. Panel war zu lange offen) -> stiller Abbruch statt Crash
        print("[PANEL] Interaction abgelaufen, edit verworfen.")
    except discord.HTTPException as e:
        if getattr(e, "code", None) == 40060 or "already been acknowledged" in str(e):
            try:
                await interaction.edit_original_response(**kwargs)
            except discord.HTTPException as e2:
                print(f"[DEDUPE] Interaction-Edit verworfen (Doppel-Dispatch): {e2}")
        else:
            raise
...
@app_commands.describe(
    ...
    kanal="Kanal, in den Teamwarn-Benachrichtigungen gesendet werden (optional, sonst im Ausführungskanal)"
)
...
async def teamwarn_setup(
    ...
    cfg = await load_config()
    tw_cfg = dict(cfg.get("teamwarn_config", {}).get(str(interaction.guild.id), {}))
    tw_cfg["von_rolle_id"] = str(von_rolle.id)
    tw_cfg["bis_rolle_id"] = str(bis_rolle.id)
    if kanal:
        tw_cfg["notif_channel_id"] = str(kanal.id)

    info_lines = [f"📊 **Rollen-Bereich:** {von_rolle.mention} → {bis_rolle.mention}"]
    if kanal:
        info_lines.append(f"📢 **Benachrichtigungskanal:** {kanal.mention}")
...
async def teamwarn(interaction: discord.Interaction, member: discord.Member, grund: str = "Kein Grund"):
    ...
    notif_channel_id = tw_cfg.get("notif_channel_id")
    notif_channel = interaction.guild.get_channel(int(notif_channel_id)) if notif_channel_id else None
    if notif_channel:
        await notif_channel.send(embed=embed, allowed_mentions=discord.AllowedMentions.none())
        await interaction.followup.send(f"✅ Verwarnung #{count} wurde in {notif_channel.mention} gepostet.", ephemeral=True)
    else:
...
async def _apply_warn_roles(guild: discord.Guild, member: discord.Member, tw_cfg: dict, new_count: int):
    """Setzt die Warn-Rolle passend zur neuen Verwarnungsanzahl (entfernt alte, vergibt passende neue)."""
    warn_rolle_ids = tw_cfg.get("warn_rolle_ids", [])
    if not warn_rolle_ids:
        return
    for rid in warn_rolle_ids:
        rolle = guild.get_role(int(rid))
        if rolle and rolle in member.roles:
            try:
                await member.remove_roles(rolle, reason="Teamwarn-Remove: Warn-Rollen Update")
            except Exception:
                pass
    if new_count > 0 and new_count <= len(warn_rolle_ids):
        neue_rolle = guild.get_role(int(warn_rolle_ids[new_count - 1]))
        if neue_rolle:
            try:
                await member.add_roles(neue_rolle, reason=f"Teamwarn-Remove: {new_count} Verwarnung(en) verbleibend")
            except Exception:
                pass


class TeamwarnRemoveView(discord.ui.View):
    def __init__(self, member: discord.Member, count: int):
        super().__init__(timeout=60)
        self.member = member
        self.count = count

    @discord.ui.button(label="Eine Verwarnung entfernen", style=discord.ButtonStyle.primary, emoji="➖")
    async def remove_one(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if not user_warnings:
            await interaction.followup.send("❌ Dieser User hat keine Verwarnungen mehr.", ephemeral=True)
            return
        user_warnings.pop()
        ...
        cfg = await load_config()
        tw_cfg = cfg.get("teamwarn_config", {}).get(str(interaction.guild.id), {})
        await _apply_warn_roles(interaction.guild, self.member, tw_cfg, len(user_warnings))
        ...
        await interaction.edit_original_response(
            embed=liquid_glass_embed(
                "➖ Verwarnung entfernt",
                f"**{self.member}** hat jetzt **{len(user_warnings)}** Verwarnung(en).",
                discord.Color.from_rgb(255, 165, 0)
            ),
            view=self
        )

    @discord.ui.button(label="Alle Verwarnungen entfernen", style=discord.ButtonStyle.danger, emoji="🗑️")
    async def remove_all(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        cfg = await load_config()
        tw_cfg = cfg.get("teamwarn_config", {}).get(str(interaction.guild.id), {})
        await _apply_warn_roles(interaction.guild, self.member, tw_cfg, 0)

        for item in self.children:
            item.disabled = True
        await interaction.edit_original_response(
            embed=liquid_glass_embed(
                "🗑️ Alle Verwarnungen entfernt",
                f"**{self.member}** hat jetzt **0** Verwarnungen.",
                discord.Color.from_rgb(100, 220, 150)
            ),
            view=self
        )
    ...
    async def cancel(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await interaction.response.edit_message(
            embed=liquid_glass_embed("❌ Abgebrochen", "Es wurde nichts geändert.", discord.Color.from_rgb(150, 150, 150)),
            view=self
        )


@tree.command(name="teamwarn-remove", description="Entfernt Verwarnungen eines Users (Admin)")
@app_commands.describe(member="Der User, dessen Verwarnungen bearbeitet werden sollen")
async def teamwarn_remove(interaction: discord.Interaction, member: discord.Member):
    if not has_permission(interaction, "administrator"):
        await interaction.response.send_message("Keine Berechtigung!", ephemeral=True)
        return
    fresh_warnings = await load_warnings()
    ...
    if count == 0:
        await interaction.response.send_message(f"**{member}** hat aktuell keine Verwarnungen.", ephemeral=True)
        return

    embed = liquid_glass_embed(
        "⚠️ Verwarnungen bearbeiten",
        f"**{member}** hat aktuell **{count}** Verwarnung(en).\n\nWas möchtest du tun?",
        discord.Color.from_rgb(255, 200, 60)
    )
    await interaction.response.send_message(embed=embed, view=TeamwarnRemoveView(member, count), ephemeral=True)
...
class TicketWeiterleitenSelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if not new_user:
            await safe_interaction_response(interaction, content="❌ User nicht gefunden!", ephemeral=True)
            return
        try:
            ...
            await safe_interaction_response(interaction, content=f"❌ Fehler: {e}", ephemeral=True)
            return
...
class TicketCloseView(discord.ui.View):
    ...
    async def close_ticket(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if not can_close:
            await safe_interaction_response(interaction, content="Keine Berechtigung!", ephemeral=True)
            return
...
class TicketNotifView(discord.ui.View):
    ...
    async def _accept(self, interaction: discord.Interaction, button: discord.ui.Button, channel_id: str):
        if _dedupe_interaction(interaction):
            return  # Doppelte Interaction (z.B. nach Gateway-Resume) – bereits verarbeitet
        cfg = await get_ticket_config(interaction.guild.id)
        support_role_id = cfg.get("support_role")
        zustaendig_role_id = cfg.get("zustaendig_role")
        support_role = interaction.guild.get_role(int(support_role_id)) if support_role_id else None
        ...
        if not can_accept:
            await safe_interaction_response(interaction, content="Du hast keine Berechtigung dieses Ticket anzunehmen!", ephemeral=True)
            return

        ticket_channel = interaction.guild.get_channel(int(channel_id))
        if not ticket_channel:
            await safe_interaction_response(interaction, content="❌ Ticket-Kanal nicht mehr vorhanden.", ephemeral=True)
            return
        ...
        await safe_interaction_response(interaction, content=f"✅ Du hast das Ticket {ticket_channel.mention} angenommen!", ephemeral=True)
    ...
    async def _close(self, interaction: discord.Interaction, channel_id: str):
        ...
        ticket_channel = interaction.guild.get_channel(int(channel_id))
        if not ticket_channel:
            await safe_interaction_response(interaction, content="❌ Ticket-Kanal nicht mehr vorhanden (evtl. schon geschlossen).", ephemeral=True)
            return
    ...
    async def _close(self, interaction: discord.Interaction, delete: bool):
        ...
        if delete:
            ...
            try:
                ...
                pass  # Channel wurde bereits gelöscht (z.B. Doppel-Klick oder manuell entfernt)
            except Exception as e:
...
class VoiceSupportView(discord.ui.View):
    ...
    async def _decline_callback(self, interaction: discord.Interaction):
        ...
        if not has_support_role(interaction, self.support_role_id):
            await safe_interaction_response(
                interaction,
                content="❌ Du hast keine Berechtigung, Supports abzulehnen!\n"
                f"Benötigt: <@&{self.support_role_id}>",
                ephemeral=True
            )
            return
...
class VoiceSupportCloseView(discord.ui.View):
    ...
    async def _close_callback(self, interaction: discord.Interaction):
        ...
        if self.acceptor_id and interaction.user.id != self.acceptor_id and not interaction.user.guild_permissions.administrator:
            await safe_interaction_response(
                interaction,
                content="❌ Nur die Person die den Support angenommen hat kann ihn schließen!",
                ephemeral=True
            )
            return
        guild = interaction.guild
        ...
        embed_fertig = liquid_glass_embed(
            "✅ Support beendet",
        ...
        try:
            if interaction.response.is_done():
                await interaction.edit_original_response(embed=embed_fertig, view=self)
            else:
                await interaction.response.edit_message(embed=embed_fertig, view=self)
        except discord.HTTPException as e:
            print(f"[VS] _close_callback edit fehlgeschlagen: {e}")
        ...
        for fmt in ["%d.%m.%Y", "%d.%m.%y", "%d.%m."]:
            try:
                if fmt == "%d.%m.":
                    ...
                    parsed = p.replace(year=_dt.now(timezone.utc).year)
                else:
...
async def get_updownrank_config(guild_id: int) -> dict:
    cfg = await load_config()
    return cfg.get("updownrank", {}).get(str(guild_id), {})

async def save_updownrank_config(guild_id: int, data: dict):
    cfg = await load_config()
    if "updownrank" not in cfg:
        cfg["updownrank"] = {}
    cfg["updownrank"][str(guild_id)] = data
    await save_config(cfg)
                ...
                try:
                    ...
                    if log_channel_id:
                        ...
                        if log_ch:
                            ...
                            ts = _dt.datetime.now(_dt.timezone.utc).strftime("%d.%m.%Y um %H:%M Uhr")
                            try:
                                await log_ch.send(
                                    embed=liquid_glass_embed(
...
async def backup_erstellen(interaction: discord.Interaction):
    ...
    try:
        ...
        import datetime
        ts = datetime.datetime.fromtimestamp(backup_data["timestamp"], datetime.timezone.utc).strftime("%d.%m.%Y um %H:%M Uhr")
        await interaction.followup.send(
            embed=liquid_glass_embed(
                "💾 Backup erstellt",
...
async def backup_liste(interaction: discord.Interaction):
    ...
    for i, b in enumerate(backups, 1):
        ts = datetime.datetime.fromtimestamp(b["timestamp"], datetime.timezone.utc).strftime("%d.%m.%Y %H:%M Uhr")
...
async def backup_laden(interaction: discord.Interaction, nummer: int):
    ...
    import datetime
    ts = datetime.datetime.fromtimestamp(backup["timestamp"], datetime.timezone.utc).strftime("%d.%m.%Y %H:%M Uhr")
...
@tree.command(name="uprank-downrank-setup", description="Legt den Kanal für Uprank/Downrank-Benachrichtigungen fest (Admin)")
@app_commands.describe(kanal="Kanal, in den /uprank und /downrank Benachrichtigungen gesendet werden")
async def uprank_downrank_setup(interaction: discord.Interaction, kanal: discord.TextChannel):
    if not interaction.user.guild_permissions.administrator:
        await interaction.response.send_message("❌ Keine Berechtigung!", ephemeral=True)
        return
    await interaction.response.defer(ephemeral=True)
    await save_updownrank_config(interaction.guild.id, {"channel_id": str(kanal.id)})
    await interaction.followup.send(
        embed=liquid_glass_embed(
            "✅ Uprank/Downrank-Kanal gesetzt",
            f"Benachrichtigungen für `/uprank` und `/downrank` werden ab jetzt in {kanal.mention} gepostet.",
            discord.Color.from_rgb(100, 220, 150)
        ),
        ephemeral=True
    )
...
async def uprank(interaction: discord.Interaction, wer: discord.Member, von_rolle: discord.Role, auf_rolle: discord.Role, grund: str = "Kein Grund angegeben"):
    ...
    await interaction.followup.send(embed=embed)
    # Uprank/Downrank Benachrichtigungskanal
    ud_cfg = await get_updownrank_config(interaction.guild.id)
    ud_ch_id = ud_cfg.get("channel_id")
    if ud_ch_id:
        ud_ch = interaction.guild.get_channel(int(ud_ch_id))
        if ud_ch:
            try:
                await ud_ch.send(embed=embed)
            except Exception:
                pass
    else:
...
# ═══════════════════════════════════════════════
# OWNER-PANEL SYSTEM (/panel) – nur für den Bot-Owner
# ═══════════════════════════════════════════════

# ── Apply-Funktionen: führen die eigentliche Setup-Logik für einen Ziel-Server aus ──

async def panel_apply_teamwarn(guild: discord.Guild, v: dict, invoker_id: int):
    cfg = await load_config()
    tw_cfg = dict(cfg.get("teamwarn_config", {}).get(str(guild.id), {}))
    von_rolle = guild.get_role(int(v["von_rolle"]))
    bis_rolle = guild.get_role(int(v["bis_rolle"]))
    tw_cfg["von_rolle_id"] = str(von_rolle.id)
    tw_cfg["bis_rolle_id"] = str(bis_rolle.id)
    kanal = guild.get_channel(int(v["kanal"])) if v.get("kanal") else None
    if kanal:
        tw_cfg["notif_channel_id"] = str(kanal.id)
    ...
    if v.get("warn_rollen_erstellen", "ja") == "ja":
        warn_rollen = [
            ("🟡 1. Verwarnung", discord.Color.from_rgb(255, 220, 0)),
            ("🟠 2. Verwarnung", discord.Color.from_rgb(255, 140, 0)),
            ("🔴 3. Verwarnung", discord.Color.from_rgb(220, 40, 40)),
        ...
        for name, color in warn_rollen:
            ...
            if existing:
                ...
                new_role = await guild.create_role(name=name, color=color, reason="Teamwarn-System Setup (Panel)")
                role_ids.append(str(new_role.id))
                info_lines.append(f"✅ Erstellt: {new_role.mention}")
        tw_cfg["warn_rolle_ids"] = role_ids

    cfg.setdefault("teamwarn_config", {})[str(guild.id)] = tw_cfg
    await save_config(cfg)
    return liquid_glass_embed("✅ Teamwarn-System eingerichtet", "\n".join(info_lines), discord.Color.from_rgb(100, 220, 150))


async def panel_apply_ticket(guild: discord.Guild, v: dict, invoker_id: int):
    kanal = guild.get_channel(int(v["kanal"]))
    ticket_kategorie = guild.get_channel(int(v["ticket_kategorie"]))
    transcript_kanal = guild.get_channel(int(v["transcript_kanal"]))
    support_rolle = guild.get_role(int(v["support_rolle"]))
    benachrichtigungs_kanal = guild.get_channel(int(v["benachrichtigungs_kanal"]))
    ping_rolle = guild.get_role(int(v["ping_rolle"])) if v.get("ping_rolle") else None
    archiv_kategorie = guild.get_channel(int(v["archiv_kategorie"])) if v.get("archiv_kategorie") else None
    modal_aktiviert = v.get("modal_aktiviert", "nein")
    modal_frage = v.get("modal_frage")
    ...
    if not archiv_kategorie:
        try:
            ...
            return liquid_glass_embed("❌ Fehler", f"Archiv-Kategorie konnte nicht erstellt werden: `{e}`", discord.Color.from_rgb(220, 60, 60))
    ...
    return liquid_glass_embed(
        "✅ Ticket-System eingerichtet!",
        f"**Panel:** {kanal.mention}\n**Ticket-Kategorie:** {ticket_kategorie.name}\n**Archiv-Kategorie:** {archiv_kategorie.name}\n**Transcript:** {transcript_kanal.mention}\n**Support-Rolle:** {support_rolle.mention}{ping_info}\n**Benachrichtigungen:** {benachrichtigungs_kanal.mention}",
        discord.Color.from_rgb(100, 220, 150)
    )


async def panel_apply_voice1(guild: discord.Guild, v: dict, invoker_id: int):
    warteraum = guild.get_channel(int(v["warteraum"]))
    benachrichtigungs_kanal = guild.get_channel(int(v["benachrichtigungs_kanal"]))
    support_kategorie = guild.get_channel(int(v["support_kategorie"]))
    ping_rolle = guild.get_role(int(v["ping_rolle"]))
    support_rolle = guild.get_role(int(v["support_rolle"]))
    data = {
        "warteraum_id": str(warteraum.id),
        "notif_channel_id": str(benachrichtigungs_kanal.id),
    ...
    await save_voice_support_config(guild.id, data)
    room_count = len(support_kategorie.voice_channels)
    return liquid_glass_embed(
        "✅ Voice-Support eingerichtet!",
        f"**Warteraum:** {warteraum.mention}\n**Benachrichtigungen:** {benachrichtigungs_kanal.mention}\n**Support-Kategorie:** {support_kategorie.name} ({room_count} Räume)\n**Ping-Rolle:** {ping_rolle.mention}\n**Support-Rolle:** {support_rolle.mention}",
...
async def panel_apply_voice2(guild: discord.Guild, v: dict, invoker_id: int):
    warteraum = guild.get_channel(int(v["warteraum"]))
    benachrichtigungs_kanal = guild.get_channel(int(v["benachrichtigungs_kanal"]))
    support_kategorie = guild.get_channel(int(v["support_kategorie"]))
    ping_rolle = guild.get_role(int(v["ping_rolle"]))
    existing = await get_voice_support_2_config(guild.id)
    data = {
        "warteraum_id": str(warteraum.id),
        "notif_channel_id": str(benachrichtigungs_kanal.id),
        "support_category_id": str(support_kategorie.id),
    ...
    await save_voice_support_2_config(guild.id, data)
    room_count = len(support_kategorie.voice_channels)
    return liquid_glass_embed(
        "✅ Voice-Support 2 eingerichtet!",
        f"**Warteraum:** {warteraum.mention}\n**Benachrichtigungen:** {benachrichtigungs_kanal.mention}\n**Support-Kategorie:** {support_kategorie.name} ({room_count} Räume)\n**Ping-Rolle:** {ping_rolle.mention}\n\n⚠️ Benutze `/voice-support-2-rolle` um die Support-Rolle einzustellen!",
...
async def panel_apply_log(guild: discord.Guild, v: dict, invoker_id: int):
    cfg = await get_log_config(guild.id)
    lines = []
    for key, label, emoji in [
        ...
        ("ban_log", "Ban-Log", "🔨"),
        ("voice_log", "Voice-Log", "🎙️"),
    ]:
        if v.get(key):
            ch = guild.get_channel(int(v[key]))
            if ch:
                cfg[key] = str(ch.id)
                lines.append(f"{emoji} {label}: {ch.mention}")
    await save_log_config(guild.id, cfg)
    return liquid_glass_embed("✅ Log-Setup", "\n".join(lines) or "Nichts geändert.", discord.Color.from_rgb(100, 220, 150))


async def panel_apply_willkommen(guild: discord.Guild, v: dict, invoker_id: int):
    kanal = guild.get_channel(int(v["kanal"]))
    nachricht = v.get("nachricht")
    bild_url = v.get("bild_url")
    cfg = await load_config()
    cfg.setdefault("welcome", {})[str(guild.id)] = {
        "channel_id": str(kanal.id),
        "message": nachricht or "Willkommen {user} auf **{server}**! Du bist Mitglied #{count}. 🎉",
        "image_url": bild_url or None
    }
    await save_config(cfg)
    return liquid_glass_embed(
        "✅ Willkommen-Setup gespeichert",
        f"**Kanal:** {kanal.mention}\n**Bild:** {'✅ gesetzt' if bild_url else '❌ kein Bild'}",
        discord.Color.from_rgb(100, 220, 150)
    )


async def panel_apply_abschied(guild: discord.Guild, v: dict, invoker_id: int):
    kanal = guild.get_channel(int(v["kanal"]))
    nachricht = v.get("nachricht")
    bild_url = v.get("bild_url")
    cfg = await load_config()
    cfg.setdefault("goodbye", {})[str(guild.id)] = {
        "channel_id": str(kanal.id),
        "message": nachricht or "**{name}** hat **{server}** verlassen. 👋",
        "image_url": bild_url or None
    }
    await save_config(cfg)
    return liquid_glass_embed(
        "✅ Abschied-Setup gespeichert",
        f"**Kanal:** {kanal.mention}\n**Bild:** {'✅ gesetzt' if bild_url else '❌ kein Bild'}",
        discord.Color.from_rgb(100, 220, 150)
    )


async def panel_apply_modlog(guild: discord.Guild, v: dict, invoker_id: int):
    kanal = guild.get_channel(int(v["kanal"]))
    log_cfg = await get_log_config(guild.id)
    log_cfg["ban_log"] = str(kanal.id)
    await save_log_config(guild.id, log_cfg)
    return liquid_glass_embed("✅ Modlog eingerichtet", f"Bans und Kicks werden ab jetzt in {kanal.mention} eingetragen.", discord.Color.from_rgb(100, 220, 150))


async def panel_apply_ingamelog(guild: discord.Guild, v: dict, invoker_id: int):
    panel_kanal = guild.get_channel(int(v["panel_kanal"]))
    log_kanal = guild.get_channel(int(v["log_kanal"]))
    auto_repost = v.get("auto_repost", "no")
    interval_stunden = int(v.get("interval_stunden") or 24)
    ...
    cfg = await load_config()
    cfg.setdefault("ingame_log_channel", {})[str(guild.id)] = str(log_kanal.id)
    await save_config(cfg)

    if auto_repost == "yes":
        await start_repost_task(guild.id, panel_kanal.id, "ingame", str(log_kanal.id), interval_stunden)
        repost_info = f"\n**Auto-Repost:** Alle **{interval_stunden} Stunden**"
    else:
        repost_info = "\n**Auto-Repost:** Deaktiviert"
    return liquid_glass_embed(
...
async def panel_apply_abmeldung(guild: discord.Guild, v: dict, invoker_id: int):
    panel_kanal = guild.get_channel(int(v["panel_kanal"]))
    log_kanal = guild.get_channel(int(v["log_kanal"]))
    auto_repost = v.get("auto_repost", "no")
    interval_stunden = int(v.get("interval_stunden") or 24)
    ...
    cfg = await load_config()
    cfg.setdefault("abmeldung_log_channel", {})[str(guild.id)] = str(log_kanal.id)
    await save_config(cfg)

    if auto_repost == "yes":
        await start_repost_task(guild.id, panel_kanal.id, "abmeldung", str(log_kanal.id), interval_stunden)
        repost_info = f"\n**Auto-Repost:** Alle **{interval_stunden} Stunden**"
    else:
        repost_info = "\n**Auto-Repost:** Deaktiviert"
    return liquid_glass_embed(
...
async def panel_apply_abmeldung_rolle(guild: discord.Guild, v: dict, invoker_id: int):
    rolle = guild.get_role(int(v["abwesenheits_rolle"]))
    cfg = await load_config()
    cfg.setdefault("abmeldung_abwesenheitsrolle", {})[str(guild.id)] = str(rolle.id)
    await save_config(cfg)
    mentionable_info = ""
    try:
        if rolle.mentionable:
            await rolle.edit(mentionable=False, reason="Abwesenheitsrolle – nicht pingbar (Panel)")
            mentionable_info = "\n🔕 Die Rolle wurde automatisch auf **nicht pingbar** gesetzt."
        else:
            mentionable_info = "\n🔕 Die Rolle ist bereits **nicht pingbar**."
    except discord.Forbidden:
    ...
    return liquid_glass_embed(
        "✅ Abwesenheitsrolle gesetzt",
        f"**Rolle:** {rolle.mention}{mentionable_info}",
        discord.Color.from_rgb(100, 220, 150)
    )


async def panel_apply_abmeldung_bestaetigung(guild: discord.Guild, v: dict, invoker_id: int):
    rolle = guild.get_role(int(v["rolle"]))
    cfg = await load_config()
    cfg.setdefault("abmeldung_bestaetigung_rolle", {})[str(guild.id)] = str(rolle.id)
    await save_config(cfg)
    return liquid_glass_embed("✅ Bestätigungsrolle gesetzt", f"**Rolle:** {rolle.mention}", discord.Color.from_rgb(100, 220, 150))


async def panel_apply_ranklog(guild: discord.Guild, v: dict, invoker_id: int):
    kanal = guild.get_channel(int(v["kanal"]))
    von = guild.get_role(int(v["von"]))
    bis = guild.get_role(int(v["bis"]))
    await save_ranklog_config(guild.id, {"channel_id": str(kanal.id), "von_role_id": str(von.id), "bis_role_id": str(bis.id)})
    min_pos = min(von.position, bis.position)
    max_pos = max(von.position, bis.position)
    erfasste_rollen = [r for r in guild.roles if min_pos <= r.position <= max_pos]
    rollen_liste = " • ".join([r.mention for r in sorted(erfasste_rollen, key=lambda r: r.position)]) or "Keine"
    return liquid_glass_embed(
        "✅ Ranklog eingerichtet!",
        f"**Kanal:** {kanal.mention}\n**Von:** {von.mention}\n**Bis:** {bis.mention}\n\n**Erfasste Rollen:**\n{rollen_liste}",
        discord.Color.from_rgb(100, 220, 150)
...
async def panel_apply_teamliste(guild: discord.Guild, v: dict, invoker_id: int):
    kanal = guild.get_channel(int(v["kanal"]))
    titel = v.get("titel") or "Teamliste"
    embed = discord.Embed(title=titel, description="Die Teamliste wird geladen...", color=discord.Color.from_rgb(140, 210, 255))
    cfg = await get_teamliste_config(guild.id)
    old_channel_id = cfg.get("channel_id")
    old_message_id = cfg.get("message_id")
    if old_channel_id and old_message_id:
...
async def panel_apply_backup(guild: discord.Guild, v: dict, invoker_id: int):
    auto_backup = v.get("auto_backup", "false") == "true"
    log_kanal = guild.get_channel(int(v["log_kanal"])) if v.get("log_kanal") else None
    data = {"auto_backup": auto_backup}
    if log_kanal:
        data["log_channel_id"] = str(log_kanal.id)
    await save_backup_config(guild.id, data)
    status = "✅ aktiviert" if auto_backup else "❌ deaktiviert"
    kanal_text = f"\n**Log-Kanal:** {log_kanal.mention}" if log_kanal else ""
    return liquid_glass_embed("💾 Backup-System eingerichtet", f"**Auto-Backup:** {status} (täglich um 03:00 Uhr){kanal_text}", discord.Color.from_rgb(100, 180, 255))


async def panel_apply_level(guild: discord.Guild, v: dict, invoker_id: int):
    levelup_kanal = guild.get_channel(int(v["levelup_kanal"]))
    owner_unendlich_xp = v.get("owner_unendlich_xp", "nein")
    if owner_unendlich_xp == "ja" and invoker_id != OWNER_ID:
        return liquid_glass_embed("❌ Fehler", "Diese Option ist nur für den Bot-Owner!", discord.Color.from_rgb(220, 60, 60))
    cfg = await get_level_config(guild.id)
    role_ids = cfg.get("role_ids", {})
    created = []
    for lvl, name, color in LEVEL_ROLES:
        if str(lvl) in role_ids:
            existing = guild.get_role(int(role_ids[str(lvl)]))
            if existing:
                continue
        try:
            role = await guild.create_role(name=name, color=discord.Color(color), reason="Level-System Setup (Panel)")
            role_ids[str(lvl)] = str(role.id)
            created.append(f"Level {lvl}: {role.mention}")
        except Exception as e:
            created.append(f"Level {lvl}: ❌ Fehler ({e})")
    ...
    xp_pro_nachricht = v.get("xp_pro_nachricht")
    xp_pro_voice = v.get("xp_pro_voice")
    if xp_pro_nachricht not in (None, ""):
        save_data["xp_pro_nachricht"] = int(xp_pro_nachricht)
    if xp_pro_voice not in (None, ""):
        save_data["xp_pro_voice"] = int(xp_pro_voice)
    await save_level_config(guild.id, save_data)
    xp_info = ""
    if xp_pro_nachricht not in (None, ""):
        xp_info += f"\n**XP pro Nachricht:** {xp_pro_nachricht}"
    if xp_pro_voice not in (None, ""):
...
async def panel_apply_updownrank(guild: discord.Guild, v: dict, invoker_id: int):
    kanal = guild.get_channel(int(v["kanal"]))
    await save_updownrank_config(guild.id, {"channel_id": str(kanal.id)})
    return liquid_glass_embed(
        "✅ Uprank/Downrank-Kanal gesetzt",
        f"Benachrichtigungen für `/uprank` und `/downrank` werden ab jetzt in {kanal.mention} gepostet.",
        discord.Color.from_rgb(100, 220, 150)
...
# ── Deklarative Feld-Spezifikationen für jedes Setup-System ──

SETUP_SPECS = {
    "teamwarn": {
        "label": "⚠️ Teamwarn-System",
        "fields": [
            {"name": "von_rolle", "kind": "role", "label": "Von-Rolle (niedrigste)"},
            {"name": "bis_rolle", "kind": "role", "label": "Bis-Rolle (höchste)"},
            {"name": "warn_rollen_erstellen", "kind": "choice", "label": "Warn-Rollen automatisch erstellen?",
             "choices": [("Ja – automatisch erstellen", "ja"), ("Nein", "nein")]},
            {"name": "kanal", "kind": "text_channel", "label": "Benachrichtigungskanal", "optional": True},
        ],
        "apply": panel_apply_teamwarn,
    },
    "ticket": {
        "label": "🎫 Ticket-System",
        "fields": [
            {"name": "kanal", "kind": "text_channel", "label": "Panel-Kanal"},
            {"name": "ticket_kategorie", "kind": "category", "label": "Ticket-Kategorie"},
            {"name": "transcript_kanal", "kind": "text_channel", "label": "Transcript-Kanal"},
            {"name": "support_rolle", "kind": "role", "label": "Support-Rolle"},
            {"name": "benachrichtigungs_kanal", "kind": "text_channel", "label": "Benachrichtigungskanal"},
            {"name": "ping_rolle", "kind": "role", "label": "Ping-Rolle", "optional": True},
            {"name": "archiv_kategorie", "kind": "category", "label": "Archiv-Kategorie", "optional": True},
            {"name": "modal_aktiviert", "kind": "choice", "label": "Formular beim Erstellen anzeigen?",
             "choices": [("Ja", "ja"), ("Nein", "nein")]},
            {"name": "modal_frage", "kind": "text", "label": "Formular-Frage", "optional": True},
        ],
        "apply": panel_apply_ticket,
    },
    "voice1": {
        "label": "🎤 Voice-Support-System",
        "fields": [
            {"name": "warteraum", "kind": "voice_channel", "label": "Warteraum"},
            {"name": "benachrichtigungs_kanal", "kind": "text_channel", "label": "Benachrichtigungskanal"},
            {"name": "support_kategorie", "kind": "category", "label": "Support-Kategorie"},
            {"name": "ping_rolle", "kind": "role", "label": "Ping-Rolle"},
            {"name": "support_rolle", "kind": "role", "label": "Support-Rolle"},
        ],
        "apply": panel_apply_voice1,
    },
    "voice2": {
        "label": "🎤 Voice-Support-System 2",
        "fields": [
            {"name": "warteraum", "kind": "voice_channel", "label": "Warteraum"},
            {"name": "benachrichtigungs_kanal", "kind": "text_channel", "label": "Benachrichtigungskanal"},
            {"name": "support_kategorie", "kind": "category", "label": "Support-Kategorie"},
        ...
        "apply": panel_apply_voice2,
    },
    "log": {
        "label": "📝 Log-System",
        "fields": [
            {"name": "message_log", "kind": "text_channel", "label": "Message-Log", "optional": True},
            {"name": "join_leave_log", "kind": "text_channel", "label": "Join/Leave-Log", "optional": True},
            {"name": "ban_log", "kind": "text_channel", "label": "Ban-Log", "optional": True},
            {"name": "voice_log", "kind": "text_channel", "label": "Voice-Log", "optional": True},
        ],
        "apply": panel_apply_log,
    },
    "willkommen": {
        "label": "👋 Willkommensnachricht",
        "fields": [
            {"name": "kanal", "kind": "text_channel", "label": "Willkommens-Kanal"},
            {"name": "nachricht", "kind": "text", "label": "Nachricht (leer = Standard)", "optional": True, "long": True},
            {"name": "bild_url", "kind": "text", "label": "Bild-URL", "optional": True},
        ],
        "apply": panel_apply_willkommen,
    },
    "abschied": {
        "label": "👋 Abschiedsnachricht",
        "fields": [
            {"name": "kanal", "kind": "text_channel", "label": "Abschieds-Kanal"},
            {"name": "nachricht", "kind": "text", "label": "Nachricht (leer = Standard)", "optional": True, "long": True},
            {"name": "bild_url", "kind": "text", "label": "Bild-URL", "optional": True},
        ],
        "apply": panel_apply_abschied,
    },
    "modlog": {
        "label": "🔨 Modlog (Bans/Kicks)",
        "fields": [
            {"name": "kanal", "kind": "text_channel", "label": "Modlog-Kanal"},
        ],
        "apply": panel_apply_modlog,
    },
    "ingamelog": {
        "label": "🎮 In-Game Log",
        "fields": [
            {"name": "panel_kanal", "kind": "text_channel", "label": "Panel-Kanal"},
            {"name": "log_kanal", "kind": "text_channel", "label": "Log-Kanal"},
            {"name": "auto_repost", "kind": "choice", "label": "Panel automatisch neu senden?",
             "choices": [("Ja", "yes"), ("Nein", "no")]},
            {"name": "interval_stunden", "kind": "int", "label": "Intervall in Stunden (Standard 24)", "optional": True, "default": 24},
        ],
        "apply": panel_apply_ingamelog,
    },
    "abmeldung": {
        "label": "📋 Abmeldungs-System",
        "fields": [
            {"name": "panel_kanal", "kind": "text_channel", "label": "Panel-Kanal"},
            {"name": "log_kanal", "kind": "text_channel", "label": "Log-Kanal"},
            {"name": "auto_repost", "kind": "choice", "label": "Panel automatisch neu senden?",
        ...
        "apply": panel_apply_abmeldung,
    },
    "abmeldung_rolle": {
        "label": "🔕 Abwesenheitsrolle",
        "fields": [
            {"name": "abwesenheits_rolle", "kind": "role", "label": "Abwesenheitsrolle"},
        ],
        "apply": panel_apply_abmeldung_rolle,
    },
    "abmeldung_bestaetigung": {
        "label": "✅ Abmeldungs-Bestätigungsrolle",
        "fields": [
            {"name": "rolle", "kind": "role", "label": "Bestätigungsrolle"},
        ],
        "apply": panel_apply_abmeldung_bestaetigung,
    },
    "ranklog": {
        "label": "📊 Ranklog",
        "fields": [
            {"name": "kanal", "kind": "text_channel", "label": "Ranklog-Kanal"},
            {"name": "von", "kind": "role", "label": "Von-Rolle (niedrigste)"},
            {"name": "bis", "kind": "role", "label": "Bis-Rolle (höchste)"},
        ],
        "apply": panel_apply_ranklog,
    },
    "teamliste": {
        "label": "📋 Teamliste",
        "fields": [
            {"name": "kanal", "kind": "text_channel", "label": "Teamliste-Kanal"},
            {"name": "titel", "kind": "text", "label": "Titel (Standard: Teamliste)", "optional": True, "default": "Teamliste"},
        ],
        "apply": panel_apply_teamliste,
    },
    "backup": {
        "label": "💾 Backup-System",
        "fields": [
            {"name": "auto_backup", "kind": "choice", "label": "Automatisches tägliches Backup?",
             "choices": [("Ja", "true"), ("Nein", "false")]},
            {"name": "log_kanal", "kind": "text_channel", "label": "Log-Kanal", "optional": True},
        ],
        "apply": panel_apply_backup,
    },
    "level": {
        "label": "⭐ Level-System",
        "fields": [
            {"name": "levelup_kanal", "kind": "text_channel", "label": "Level-Up-Kanal"},
            {"name": "xp_pro_nachricht", "kind": "int", "label": "XP pro Nachricht", "optional": True},
            {"name": "xp_pro_voice", "kind": "int", "label": "XP pro Voice-Minute", "optional": True},
            {"name": "owner_unendlich_xp", "kind": "choice", "label": "Owner unendlich XP? (nur Owner)",
             "choices": [("Ja", "ja"), ("Nein", "nein")]},
        ],
        "apply": panel_apply_level,
    },
    "updownrank": {
        "label": "📢 Uprank/Downrank-Kanal",
        "fields": [
            {"name": "kanal", "kind": "text_channel", "label": "Benachrichtigungskanal"},
        ],
        "apply": panel_apply_updownrank,
    },
}


# ── Aktionen (keine Einrichtung, sondern direkte Ausführung) ──

async def panel_apply_uprank(guild: discord.Guild, v: dict, invoker_id: int):
    wer = guild.get_member(int(v["wer"]))
    von_rolle = guild.get_role(int(v["von_rolle"]))
    auf_rolle = guild.get_role(int(v["auf_rolle"]))
    ...
    try:
        if von_rolle in wer.roles:
            await wer.remove_roles(von_rolle, reason=f"Uprank (Panel): {grund}")
        await wer.add_roles(auf_rolle, reason=f"Uprank (Panel): {grund}")
    except discord.Forbidden:
        return liquid_glass_embed("❌ Fehler", "Keine Berechtigung die Rollen zu ändern!", discord.Color.from_rgb(220, 60, 60))

    embed = discord.Embed(title="⬆️ Uprank", color=discord.Color.from_rgb(100, 220, 150), timestamp=datetime.now(timezone.utc))
    embed.add_field(name="👤 User", value=wer.mention, inline=True)
    ...
    embed.add_field(name="📝 Grund", value=grund, inline=False)
    embed.set_thumbnail(url=wer.display_avatar.url)
    embed.set_footer(text="GermanyRP • Uprank (Panel)")

    ud_cfg = await get_updownrank_config(guild.id)
    ud_ch_id = ud_cfg.get("channel_id")
    if ud_ch_id:
        ud_ch = guild.get_channel(int(ud_ch_id))
        if ud_ch:
            try:
                await ud_ch.send(embed=embed)
            except Exception:
    ...
    return liquid_glass_embed("✅ Uprank durchgeführt", f"{wer.mention}: {von_rolle.mention} → {auf_rolle.mention}\n**Grund:** {grund}", discord.Color.from_rgb(100, 220, 150))


async def panel_apply_downrank(guild: discord.Guild, v: dict, invoker_id: int):
    wer = guild.get_member(int(v["wer"]))
    von_rolle = guild.get_role(int(v["von_rolle"]))
    auf_rolle = guild.get_role(int(v["auf_rolle"]))
    grund = v.get("grund") or "Kein Grund angegeben"
    ...
    try:
        if von_rolle in wer.roles:
            await wer.remove_roles(von_rolle, reason=f"Downrank (Panel): {grund}")
        await wer.add_roles(auf_rolle, reason=f"Downrank (Panel): {grund}")
    except discord.Forbidden:
        return liquid_glass_embed("❌ Fehler", "Keine Berechtigung die Rollen zu ändern!", discord.Color.from_rgb(220, 60, 60))

    embed = discord.Embed(title="⬇️ Downrank", color=discord.Color.from_rgb(255, 100, 80), timestamp=datetime.now(timezone.utc))
    embed.add_field(name="👤 User", value=wer.mention, inline=True)
    embed.add_field(name="👮 Moderator", value=f"<@{invoker_id}>", inline=True)
    embed.add_field(name="📈 Von Rolle", value=von_rolle.mention, inline=True)
    embed.add_field(name="📉 Auf Rolle", value=auf_rolle.mention, inline=True)
    embed.add_field(name="📝 Grund", value=grund, inline=False)
    embed.set_thumbnail(url=wer.display_avatar.url)
    embed.set_footer(text="GermanyRP • Downrank (Panel)")
    ...
    return liquid_glass_embed("✅ Downrank durchgeführt", f"{wer.mention}: {von_rolle.mention} → {auf_rolle.mention}\n**Grund:** {grund}", discord.Color.from_rgb(100, 220, 150))


async def panel_apply_teamwarn(guild: discord.Guild, v: dict, invoker_id: int):
    member = guild.get_member(int(v["member"]))
    grund = v.get("grund") or "Kein Grund"
    if not member:
        return liquid_glass_embed("❌ Fehler", "User nicht gefunden.", discord.Color.from_rgb(220, 60, 60))
    if is_owner(member):
        return liquid_glass_embed("❌ Fehler", "Der Eigentümer ist immun!", discord.Color.from_rgb(220, 60, 60))
    ...
    if user_id not in warnings_data:
        warnings_data[user_id] = []
    invoker_user = bot.get_user(invoker_id)
    invoker_name = str(invoker_user) if invoker_user else f"<@{invoker_id}>"
    warnings_data[user_id].append({
        "reason": grund,
        "by": invoker_name,
        "at": datetime.now(timezone.utc).isoformat()
    ...
    cfg = await load_config()
    tw_cfg = cfg.get("teamwarn_config", {}).get(str(guild.id), {})
    warn_rolle_ids = tw_cfg.get("warn_rolle_ids", [])
    warn_rolle_text = ""
    ...
    if warn_rolle_ids and count <= len(warn_rolle_ids):
        for rid in warn_rolle_ids:
            ...
            if rolle and rolle in member.roles:
                try:
                    await member.remove_roles(rolle, reason="Warn-Rollen Update (Panel)")
                except Exception:
                    pass
        neue_warn_rolle = guild.get_role(int(warn_rolle_ids[count - 1]))
        if neue_warn_rolle:
            try:
                await member.add_roles(neue_warn_rolle, reason=f"Teamwarn #{count} (Panel)")
                warn_rolle_text = f"\n🏷️ **Warn-Rolle:** {neue_warn_rolle.mention}"
            except Exception:
                pass
    ...
    embed.add_field(name="👮 Moderator", value=invoker_name, inline=True)
    embed.add_field(name="📊 Verwarnungen", value=f"{count}/3", inline=True)

    notif_channel_id = tw_cfg.get("notif_channel_id")
    notif_channel = guild.get_channel(int(notif_channel_id)) if notif_channel_id else None
    posted_info = ""
    if notif_channel:
        try:
            await notif_channel.send(embed=embed, allowed_mentions=discord.AllowedMentions.none())
            posted_info = f"\n📢 Gepostet in {notif_channel.mention}"
        except Exception:
            pass
    else:
        posted_info = "\n⚠️ Kein Benachrichtigungskanal konfiguriert – nur hier sichtbar."

    result_lines = [f"**{member}** wurde verwarnt.", f"**Grund:** {grund}", f"**Verwarnungen:** {count}/3{warn_rolle_text}{posted_info}"]
    ...
    if count >= 3:
        ...
        for rid in warn_rolle_ids:
            ...
            if rolle and rolle in member.roles:
                try:
                    await member.remove_roles(rolle, reason="Teamwarn: 3 Warns erreicht (Panel)")
                except Exception:
                    pass
        if von_rolle_id and bis_rolle_id:
            von_rolle = guild.get_role(int(von_rolle_id))
            bis_rolle = guild.get_role(int(bis_rolle_id))
            if von_rolle and bis_rolle:
                min_pos = min(von_rolle.position, bis_rolle.position)
                max_pos = max(von_rolle.position, bis_rolle.position)
                roles_to_remove = [
                    r for r in member.roles
                    if r != guild.default_role and r.is_assignable() and min_pos <= r.position <= max_pos
                ]
            else:
                roles_to_remove = []
        else:
        ...
        if roles_to_remove:
            try:
                await member.remove_roles(*roles_to_remove, reason="Auto-Rollenentfernung nach 3 Verwarnungen (Panel)")
            except Exception:
                pass
        warnings_data[user_id] = []
        await save_warnings(warnings_data)
        rollen_text = ", ".join(r.mention for r in roles_to_remove) if roles_to_remove else "Keine Team-Rollen entfernt"
        result_lines.append(f"\n🔴 **3 Verwarnungen erreicht!** Entfernte Rollen: {rollen_text}")

    return liquid_glass_embed(f"✅ Verwarnung #{count} eingetragen", "\n".join(result_lines), discord.Color.from_rgb(100, 220, 150))


async def panel_apply_teamwarn_remove(guild: discord.Guild, v: dict, invoker_id: int):
    member = guild.get_member(int(v["member"]))
    aktion = v.get("aktion", "eine")
    if not member:
        return liquid_glass_embed("❌ Fehler", "User nicht gefunden.", discord.Color.from_rgb(220, 60, 60))
    fresh_warnings = await load_warnings()
    warnings_data.update(fresh_warnings)
    ...
    user_warnings = warnings_data.get(user_id, [])
    if not user_warnings:
        return liquid_glass_embed("ℹ️ Keine Verwarnungen", f"**{member}** hat aktuell keine Verwarnungen.", discord.Color.from_rgb(140, 210, 255))
    ...
    if aktion == "alle":
        warnings_data[user_id] = []
        await save_warnings(warnings_data)
        await _apply_warn_roles(guild, member, tw_cfg, 0)
        return liquid_glass_embed("🗑️ Alle Verwarnungen entfernt", f"**{member}** hat jetzt **0** Verwarnungen.", discord.Color.from_rgb(100, 220, 150))
    else:
        user_warnings.pop()
        warnings_data[user_id] = user_warnings
        await save_warnings(warnings_data)
        await _apply_warn_roles(guild, member, tw_cfg, len(user_warnings))
        return liquid_glass_embed("➖ Verwarnung entfernt", f"**{member}** hat jetzt **{len(user_warnings)}** Verwarnung(en).", discord.Color.from_rgb(255, 165, 0))


async def panel_apply_xp_geben(guild: discord.Guild, v: dict, invoker_id: int):
    user = guild.get_member(int(v["user"]))
    if not user:
        return liquid_glass_embed("❌ Fehler", "User nicht gefunden.", discord.Color.from_rgb(220, 60, 60))
    menge = int(v["menge"])
    if invoker_id != OWNER_ID and menge > 10000:
        return liquid_glass_embed("❌ Fehler", "Du kannst maximal **10.000 XP** auf einmal vergeben!", discord.Color.from_rgb(220, 60, 60))
    await add_xp(user, menge)
    doc = await get_user_xp(guild.id, user.id)
    return liquid_glass_embed(
        "✅ XP vergeben",
        f"{user.mention} hat **+{menge} XP** erhalten.\nJetzt: {doc['xp']:,} XP (Level {get_level_from_xp(doc['xp'])})",
        discord.Color.from_rgb(100, 220, 150)
...
async def panel_apply_xp_entfernen(guild: discord.Guild, v: dict, invoker_id: int):
    user = guild.get_member(int(v["user"]))
    if not user:
        return liquid_glass_embed("❌ Fehler", "User nicht gefunden.", discord.Color.from_rgb(220, 60, 60))
    menge = int(v["menge"])
    doc = await get_user_xp(guild.id, user.id)
    new_xp = max(0, doc["xp"] - menge)
    await set_user_xp(guild.id, user.id, new_xp)
    return liquid_glass_embed(
        "✅ XP entfernt",
        f"Von {user.mention} wurden **{menge} XP** entfernt.\nJetzt: {new_xp:,} XP (Level {get_level_from_xp(new_xp)})",
        discord.Color.from_rgb(220, 80, 80)
...
async def panel_apply_xp_reset_all(guild: discord.Guild, v: dict, invoker_id: int):
    if v.get("bestaetigung") != "ja":
        return liquid_glass_embed("❌ Abgebrochen", "Reset wurde nicht bestätigt, es wurde nichts gelöscht.", discord.Color.from_rgb(150, 150, 150))
    try:
        db = get_db()
        col = db["level_xp"]
        result = await col.delete_many({"guild_id": str(guild.id)})
        return liquid_glass_embed(
            "✅ Alle XP zurückgesetzt",
            f"Es wurden **{result.deleted_count} Einträge** gelöscht.\nAlle Mitglieder starten wieder bei 0 XP.",
            discord.Color.from_rgb(220, 80, 80)
        ...
        return liquid_glass_embed("❌ Fehler", f"`{e}`", discord.Color.from_rgb(220, 60, 60))


async def panel_apply_teamliste_rolle_hinzufuegen(guild: discord.Guild, v: dict, invoker_id: int):
    rolle = guild.get_role(int(v["rolle"]))
    bezeichnung = v.get("bezeichnung")
    if not rolle:
        return liquid_glass_embed("❌ Fehler", "Rolle nicht gefunden.", discord.Color.from_rgb(220, 60, 60))

    cfg = await get_teamliste_config(guild.id)
    if not cfg:
        return liquid_glass_embed("❌ Fehler", "Richte zuerst das Teamliste-Setup ein!", discord.Color.from_rgb(220, 60, 60))

    roles = cfg.get("roles", [])
    if len(roles) >= 30:
        return liquid_glass_embed("❌ Fehler", "Maximal 30 Rollen erlaubt!", discord.Color.from_rgb(220, 60, 60))
    if any(r.get("role_id") == str(rolle.id) for r in roles):
        return liquid_glass_embed("❌ Fehler", "Diese Rolle ist bereits in der Teamliste!", discord.Color.from_rgb(220, 60, 60))
...
async def panel_apply_teamliste_rolle_entfernen(guild: discord.Guild, v: dict, invoker_id: int):
    rolle = guild.get_role(int(v["rolle"]))
    if not rolle:
        return liquid_glass_embed("❌ Fehler", "Rolle nicht gefunden.", discord.Color.from_rgb(220, 60, 60))
    ...
    cfg = await get_teamliste_config(guild.id)
    if not cfg:
        return liquid_glass_embed("❌ Fehler", "Keine Teamliste eingerichtet!", discord.Color.from_rgb(220, 60, 60))

    roles = cfg.get("roles", [])
    new_roles = [r for r in roles if r.get("role_id") != str(rolle.id)]
    if len(new_roles) == len(roles):
        return liquid_glass_embed("❌ Fehler", "Diese Rolle ist nicht in der Teamliste!", discord.Color.from_rgb(220, 60, 60))
    ...
    return liquid_glass_embed(
        "✅ Rolle entfernt!",
        f"**{rolle.mention}** wurde aus der Teamliste entfernt.\nAktuell: **{len(new_roles)}/30** Rollen",
        discord.Color.from_rgb(255, 100, 100)
    )


async def panel_apply_teamliste_aktualisieren(guild: discord.Guild, v: dict, invoker_id: int):
    cfg = await get_teamliste_config(guild.id)
    if not cfg:
        return liquid_glass_embed("❌ Fehler", "Keine Teamliste eingerichtet!", discord.Color.from_rgb(220, 60, 60))
    await update_teamliste(guild)
    return liquid_glass_embed("✅ Teamliste aktualisiert!", "Die Teamliste wurde manuell neu geladen.", discord.Color.from_rgb(100, 220, 150))


async def panel_apply_kick(guild: discord.Guild, v: dict, invoker_id: int):
    member = guild.get_member(int(v["member"]))
    grund = v.get("grund") or "Kein Grund"
    if not member:
        return liquid_glass_embed("❌ Fehler", "User nicht gefunden.", discord.Color.from_rgb(220, 60, 60))
    ...
    try:
        ...
        return liquid_glass_embed("❌ Fehler", "Fehlende Berechtigung.", discord.Color.from_rgb(220, 60, 60))
    return liquid_glass_embed("✅ Kick durchgeführt", f"**{member}** wurde vom Server gekickt.\n**Grund:** {grund}", discord.Color.from_rgb(255, 160, 100))


async def panel_apply_teamkick(guild: discord.Guild, v: dict, invoker_id: int):
    member = guild.get_member(int(v["member"]))
    if not member:
        return liquid_glass_embed("❌ Fehler", "User nicht gefunden.", discord.Color.from_rgb(220, 60, 60))
    if is_owner(member):
    ...
    roles = [r for r in member.roles if r.name != "@everyone"]
    if not roles:
        return liquid_glass_embed("ℹ️ Keine Rollen", f"**{member}** hat keine Rollen.", discord.Color.from_rgb(140, 210, 255))
    try:
        await member.remove_roles(*roles, reason="Team-Kick (Panel)")
    except discord.Forbidden:
        return liquid_glass_embed("❌ Fehler", "Fehlende Berechtigung.", discord.Color.from_rgb(220, 60, 60))
    return liquid_glass_embed("✅ Team-Kick durchgeführt", f"Alle Rollen von **{member}** wurden entfernt.", discord.Color.from_rgb(255, 140, 80))


async def panel_apply_tempmute(guild: discord.Guild, v: dict, invoker_id: int):
    member = guild.get_member(int(v["member"]))
    minuten = int(v["minuten"])
    grund = v.get("grund") or "Kein Grund"
    if not member:
        return liquid_glass_embed("❌ Fehler", "User nicht gefunden.", discord.Color.from_rgb(220, 60, 60))
    if is_owner(member):
    ...
    try:
        ...
        return liquid_glass_embed("❌ Fehler", "Fehlende Berechtigung.", discord.Color.from_rgb(220, 60, 60))
    return liquid_glass_embed("✅ Temp-Mute durchgeführt", f"**{member}** wurde für **{minuten} Minuten** stummgeschaltet.\n**Grund:** {grund}", discord.Color.from_rgb(200, 100, 240))


async def panel_apply_unmute(guild: discord.Guild, v: dict, invoker_id: int):
    member = guild.get_member(int(v["member"]))
    if not member:
        return liquid_glass_embed("❌ Fehler", "User nicht gefunden.", discord.Color.from_rgb(220, 60, 60))
    if is_owner(member):
    ...
    try:
        ...
        return liquid_glass_embed("❌ Fehler", "Fehlende Berechtigung.", discord.Color.from_rgb(220, 60, 60))
    return liquid_glass_embed("✅ Unmute durchgeführt", f"Der Mute von **{member}** wurde aufgehoben.", discord.Color.from_rgb(100, 220, 150))


async def panel_apply_bann(guild: discord.Guild, v: dict, invoker_id: int):
    member = guild.get_member(int(v["member"]))
    grund = v.get("grund") or "Kein Grund"
    if not member:
        return liquid_glass_embed("❌ Fehler", "User nicht gefunden.", discord.Color.from_rgb(220, 60, 60))
    ...
    try:
        ...
        return liquid_glass_embed("❌ Fehler", "Fehlende Berechtigung.", discord.Color.from_rgb(220, 60, 60))
    return liquid_glass_embed("✅ Ban durchgeführt", f"**{member}** wurde gebannt.\n**Grund:** {grund}", discord.Color.from_rgb(220, 60, 60))


async def panel_apply_unban(guild: discord.Guild, v: dict, invoker_id: int):
    raw_id = (v.get("user_id") or "").strip()
    try:
        user = await bot.fetch_user(int(raw_id))
        await guild.unban(user)
    except Exception:
        return liquid_glass_embed("❌ Fehler", "Fehler beim Entbannen (User-ID prüfen).", discord.Color.from_rgb(220, 60, 60))
    return liquid_glass_embed("✅ Unban durchgeführt", f"**{user}** wurde entbannt.", discord.Color.from_rgb(100, 220, 150))


async def panel_apply_clear(guild: discord.Guild, v: dict, invoker_id: int):
    kanal = guild.get_channel(int(v["kanal"]))
    anzahl = int(v.get("anzahl") or 100)
    if anzahl > 100:
        anzahl = 100
    member = guild.get_member(int(v["member"])) if v.get("member") else None
    if not kanal:
        return liquid_glass_embed("❌ Fehler", "Kanal nicht gefunden.", discord.Color.from_rgb(220, 60, 60))
    try:
        if member:
            deleted = []
            async for msg in kanal.history(limit=200):
                if msg.author == member:
                    deleted.append(msg)
                if len(deleted) >= anzahl:
                    break
            for msg in deleted:
                try:
                    ...
                    pass
            count = len(deleted)
            text = f"**{count}** Nachricht(en) von **{member}** wurden gelöscht."
        else:
            deleted = await kanal.purge(limit=anzahl)
            count = len(deleted)
            text = f"**{count}** Nachricht(en) wurden gelöscht."
    except discord.Forbidden:
        return liquid_glass_embed("❌ Fehler", "Fehlende Berechtigung.", discord.Color.from_rgb(220, 60, 60))
    return liquid_glass_embed("✅ Clear durchgeführt", text, discord.Color.from_rgb(255, 100, 100))


async def panel_apply_softban(guild: discord.Guild, v: dict, invoker_id: int):
    mitglied = guild.get_member(int(v["mitglied"]))
    grund = v.get("grund") or "Kein Grund angegeben"
    if not mitglied:
        return liquid_glass_embed("❌ Fehler", "User nicht gefunden.", discord.Color.from_rgb(220, 60, 60))
    try:
        await mitglied.ban(reason=f"Softban: {grund}", delete_message_days=7)
        await guild.unban(mitglied, reason="Softban abgeschlossen")
    except discord.Forbidden:
        return liquid_glass_embed("❌ Fehler", "Fehlende Berechtigung.", discord.Color.from_rgb(220, 60, 60))
    return liquid_glass_embed("✅ Softban durchgeführt", f"**{mitglied}** wurde soft-gebannt.\n**Grund:** {grund}", discord.Color.from_rgb(255, 140, 0))


async def panel_apply_massban(guild: discord.Guild, v: dict, invoker_id: int):
    user_ids = v.get("user_ids", "") or ""
    grund = v.get("grund") or "Massban"
    ids = [uid.strip() for uid in user_ids.split(",") if uid.strip()]
    success, failed = [], []
    for uid in ids:
        try:
            user = discord.Object(id=int(uid))
            await guild.ban(user, reason=grund)
            success.append(uid)
        except Exception:
            failed.append(uid)
    msg = f"✅ Gebannt: {len(success)}\n❌ Fehlgeschlagen: {len(failed)}"
    return liquid_glass_embed("✅ Massban durchgeführt", msg, discord.Color.from_rgb(220, 50, 50))


async def panel_apply_nick(guild: discord.Guild, v: dict, invoker_id: int):
    mitglied = guild.get_member(int(v["mitglied"]))
    ...
    try:
        ...
        return liquid_glass_embed("❌ Fehler", "Fehlende Berechtigung.", discord.Color.from_rgb(220, 60, 60))
    text = f"Nickname von **{mitglied}** wurde auf **{nickname or 'Standard'}** gesetzt."
    return liquid_glass_embed("✅ Nickname geändert", text, discord.Color.from_rgb(130, 200, 240))


async def panel_apply_slowmode(guild: discord.Guild, v: dict, invoker_id: int):
    kanal = guild.get_channel(int(v["kanal"]))
    sekunden = int(v["sekunden"])
    if not kanal:
        return liquid_glass_embed("❌ Fehler", "Kanal nicht gefunden.", discord.Color.from_rgb(220, 60, 60))
    await kanal.edit(slowmode_delay=sekunden)
    text = f"Slowmode in {kanal.mention} auf **{sekunden}s** gesetzt." if sekunden > 0 else f"Slowmode in {kanal.mention} **deaktiviert**."
...
ACTION_SPECS = {
    "uprank": {
        ...
        "fields": [
            {"name": "wer", "kind": "member", "label": "User (ID oder @Erwähnung)"},
            {"name": "von_rolle", "kind": "role", "label": "Aktuelle Rolle"},
            {"name": "auf_rolle", "kind": "role", "label": "Neue (höhere) Rolle"},
            {"name": "grund", "kind": "text", "label": "Grund", "optional": True, "default": "Kein Grund angegeben"},
        ],
        "apply": panel_apply_uprank,
    },
    "downrank": {
        "label": "⬇️ Downrank ausführen",
        "fields": [
            {"name": "wer", "kind": "member", "label": "User (ID oder @Erwähnung)"},
            {"name": "von_rolle", "kind": "role", "label": "Aktuelle Rolle"},
            {"name": "auf_rolle", "kind": "role", "label": "Neue (niedrigere) Rolle"},
            {"name": "grund", "kind": "text", "label": "Grund", "optional": True, "default": "Kein Grund angegeben"},
        ],
        "apply": panel_apply_downrank,
    },
    "teamwarn": {
        "label": "⚠️ Teamwarn aussprechen",
        "fields": [
            {"name": "member", "kind": "member", "label": "User (ID oder @Erwähnung)"},
            {"name": "grund", "kind": "text", "label": "Grund", "optional": True, "default": "Kein Grund"},
        ],
        "apply": panel_apply_teamwarn,
    },
    "teamwarn_remove": {
        "label": "🗑️ Teamwarn entfernen",
        "fields": [
            {"name": "member", "kind": "member", "label": "User (ID oder @Erwähnung)"},
            {"name": "aktion", "kind": "choice", "label": "Was entfernen?",
             "choices": [("Nur eine Verwarnung", "eine"), ("Alle Verwarnungen", "alle")]},
        ],
        "apply": panel_apply_teamwarn_remove,
    },
    "xp_geben": {
        "label": "⭐ XP geben",
        "fields": [
            {"name": "user", "kind": "member", "label": "User (ID oder @Erwähnung)"},
            {"name": "menge", "kind": "int", "label": "Anzahl XP"},
        ],
        "apply": panel_apply_xp_geben,
    },
    "xp_entfernen": {
        "label": "⭐ XP entfernen",
        "fields": [
        ...
        "apply": panel_apply_xp_entfernen,
    },
    "xp_reset_all": {
        "label": "🔄 Alle XP zurücksetzen",
        "fields": [
            {"name": "bestaetigung", "kind": "choice", "label": "Wirklich ALLE XP auf diesem Server löschen?",
             "choices": [("Nein – abbrechen", "nein"), ("Ja – ich bin sicher", "ja")]},
        ],
        "apply": panel_apply_xp_reset_all,
    },
    "teamliste_rolle_hinzufuegen": {
        "label": "📋 Teamliste: Rolle hinzufügen",
        "fields": [
            {"name": "rolle", "kind": "role", "label": "Rolle"},
            {"name": "bezeichnung", "kind": "text", "label": "Anzeigename in der Liste (optional)", "optional": True},
        ],
        "apply": panel_apply_teamliste_rolle_hinzufuegen,
    },
    "teamliste_rolle_entfernen": {
        "label": "📋 Teamliste: Rolle entfernen",
        "fields": [
            {"name": "rolle", "kind": "role", "label": "Rolle"},
        ],
        "apply": panel_apply_teamliste_rolle_entfernen,
    },
    "teamliste_aktualisieren": {
        "label": "📋 Teamliste: Manuell aktualisieren",
        "fields": [],
        "apply": panel_apply_teamliste_aktualisieren,
    },
    "kick": {
        "label": "👢 Kick",
        "fields": [
        ...
        "apply": panel_apply_kick,
    },
    "teamkick": {
        "label": "👥 Team-Kick (alle Rollen entfernen)",
        "fields": [
            {"name": "member", "kind": "member", "label": "User (ID oder @Erwähnung)"},
        ],
        "apply": panel_apply_teamkick,
    },
    "tempmute": {
        "label": "🔇 Tempmute",
        "fields": [
            {"name": "member", "kind": "member", "label": "User (ID oder @Erwähnung)"},
            {"name": "minuten", "kind": "int", "label": "Dauer in Minuten"},
            {"name": "grund", "kind": "text", "label": "Grund", "optional": True, "default": "Kein Grund"},
        ],
        "apply": panel_apply_tempmute,
    },
    "unmute": {
        "label": "🔊 Unmute",
        "fields": [
    ...
    "bann": {
        ...
        "apply": panel_apply_bann,
    },
    "unban": {
        "label": "🔓 Unban",
        "fields": [
            {"name": "user_id", "kind": "text", "label": "User-ID"},
        ],
        "apply": panel_apply_unban,
    },
    "clear": {
        "label": "🧹 Clear (Nachrichten löschen)",
        "fields": [
            {"name": "kanal", "kind": "text_channel", "label": "Kanal"},
            {"name": "anzahl", "kind": "int", "label": "Anzahl (max. 100)", "optional": True, "default": 100},
            {"name": "member", "kind": "member", "label": "Nur Nachrichten dieses Users (optional)", "optional": True},
        ],
        "apply": panel_apply_clear,
    },
    "softban": {
        "label": "🔨 Softban",
        "fields": [
            {"name": "mitglied", "kind": "member", "label": "User (ID oder @Erwähnung)"},
            {"name": "grund", "kind": "text", "label": "Grund", "optional": True, "default": "Kein Grund angegeben"},
        ],
        "apply": panel_apply_softban,
    },
    "massban": {
        "label": "🔨 Massban (mehrere IDs)",
        "fields": [
            {"name": "user_ids", "kind": "text", "label": "User-IDs (durch Komma getrennt)", "long": True},
            {"name": "grund", "kind": "text", "label": "Grund", "optional": True, "default": "Massban"},
        ],
        "apply": panel_apply_massban,
    },
    "nick": {
        "label": "✏️ Nickname ändern",
        "fields": [
            {"name": "mitglied", "kind": "member", "label": "User (ID oder @Erwähnung)"},
            {"name": "nickname", "kind": "text", "label": "Neuer Nickname (leer = zurücksetzen)", "optional": True},
        ],
        "apply": panel_apply_nick,
    },
    "slowmode": {
        "label": "🐌 Slowmode setzen",
        "fields": [
            {"name": "kanal", "kind": "text_channel", "label": "Kanal"},
...
# ── Generisches Wizard-System ──

class SetupSession:
    def __init__(self, guild: discord.Guild, spec: dict, invoker_id: int):
        self.guild = guild
        self.spec = spec
...
def _panel_build_options(guild: discord.Guild, field: dict):
    kind = field["kind"]
    opts = []
    if kind == "text_channel":
        items = sorted(guild.text_channels, key=lambda c: c.position)[:25]
        opts = [discord.SelectOption(label=f"#{c.name}"[:100], value=str(c.id)) for c in items]
    elif kind == "category":
        items = sorted(guild.categories, key=lambda c: c.position)[:25]
        opts = [discord.SelectOption(label=c.name[:100], value=str(c.id)) for c in items]
    elif kind == "voice_channel":
        items = sorted(guild.voice_channels, key=lambda c: c.position)[:25]
        opts = [discord.SelectOption(label=f"🔊 {c.name}"[:100], value=str(c.id)) for c in items]
    elif kind == "role":
        items = [r for r in guild.roles if r.name != "@everyone"]
        items = sorted(items, key=lambda r: r.position, reverse=True)[:25]
        opts = [discord.SelectOption(label=r.name[:100], value=str(r.id)) for r in items]
    elif kind == "choice":
        opts = [discord.SelectOption(label=label[:100], value=value) for label, value in field["choices"]]
    if field.get("optional") and kind != "choice":
        opts.append(discord.SelectOption(label="⏭️ Überspringen (kein Wert)", value="__skip__"))
    if not opts:
        opts = [discord.SelectOption(label="⚠️ Keine Optionen verfügbar", value="__none_available__")]
    return opts


async def render_panel_step(interaction: discord.Interaction, session: "SetupSession"):
    field = session.current_field()
    if field is None:
        try:
            embed_result = await session.spec["apply"](session.guild, session.values, session.invoker_id)
        except Exception as e:
            embed_result = liquid_glass_embed("❌ Fehler beim Anwenden", f"`{e}`", discord.Color.from_rgb(220, 60, 60))
        await safe_interaction_edit(interaction, embed=embed_result, view=None)
        return
    ...
    if field.get("optional"):
        desc += "  _(optional)_"
    embed = liquid_glass_embed(
        f"⚙️ {session.spec['label']} — Schritt {step_num}/{total}",
        f"Server: **{session.guild.name}**\n\n{desc}",
        discord.Color.from_rgb(130, 200, 240)
    )
    view = PanelStepView(session)
    await safe_interaction_edit(interaction, embed=embed, view=view)


class PanelChoiceSelect(discord.ui.Select):
    def __init__(self, session: SetupSession, field: dict):
        self.session = session
        self.field = field
        options = _panel_build_options(session.guild, field)
        super().__init__(placeholder=field["label"][:100], options=options, min_values=1, max_values=1)

    async def callback(self, interaction: discord.Interaction):
        if interaction.user.id != self.session.invoker_id:
            await interaction.response.send_message("❌ Dieses Panel ist nicht für dich.", ephemeral=True)
            return
        value = self.values[0]
        if value in ("__skip__", "__none_available__"):
            self.session.values[self.field["name"]] = None
        else:
            self.session.values[self.field["name"]] = value
        self.session.step_index += 1
        await render_panel_step(interaction, self.session)
...
class PanelTextModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        raw = self.text_input.value.strip() if self.text_input.value else ""
        if self.field["kind"] == "int":
            if raw == "":
                value = self.field.get("default")
            else:
                try:
                    value = int(raw)
                except ValueError:
                    ...
                    return
        elif self.field["kind"] == "member":
            cleaned = raw.strip("<@!>")
            member = None
            if cleaned.isdigit():
                member = self.session.guild.get_member(int(cleaned))
                if not member:
                    try:
                        member = await self.session.guild.fetch_member(int(cleaned))
                    except discord.NotFound:
                        member = None
            if not member:
                await interaction.response.send_message(
                    "❌ User nicht gefunden. Bitte die User-ID oder @Erwähnung eingeben.", ephemeral=True
                )
                return
            value = str(member.id)
        else:
            value = raw if raw else self.field.get("default")
        self.session.values[self.field["name"]] = value
        self.session.step_index += 1
        await render_panel_step(interaction, self.session)


class PanelStepView(discord.ui.View):
    def __init__(self, session: SetupSession):
        ...
        if field["kind"] in ("text", "int", "member"):
            btn = discord.ui.Button(label="✏️ Wert eingeben", style=discord.ButtonStyle.primary)
            btn.callback = self.open_modal
            self.add_item(btn)
            if field.get("optional"):
                skip_btn = discord.ui.Button(label="⏭️ Überspringen", style=discord.ButtonStyle.secondary)
                skip_btn.callback = self.skip
                self.add_item(skip_btn)
        else:
            self.add_item(PanelChoiceSelect(session, field))

        cancel_btn = discord.ui.Button(label="❌ Abbrechen", style=discord.ButtonStyle.danger)
    ...
    async def open_modal(self, interaction: discord.Interaction):
        if interaction.user.id != self.session.invoker_id:
            await interaction.response.send_message("❌ Dieses Panel ist nicht für dich.", ephemeral=True)
            return
        await interaction.response.send_modal(PanelTextModal(self.session, self.field))

    async def skip(self, interaction: discord.Interaction):
        if interaction.user.id != self.session.invoker_id:
            await interaction.response.send_message("❌ Dieses Panel ist nicht für dich.", ephemeral=True)
            return
        self.session.values[self.field["name"]] = self.field.get("default")
        self.session.step_index += 1
        await render_panel_step(interaction, self.session)
    ...
    async def cancel(self, interaction: discord.Interaction):
        ...
        await safe_interaction_edit(
            interaction,
            embed=liquid_glass_embed("❌ Abgebrochen", "Setup wurde abgebrochen, es wurde nichts geändert.", discord.Color.from_rgb(150, 150, 150)),
            view=None
        )


# ── Server- und Kategorie-Auswahl ──

def build_panel_overview_embed() -> discord.Embed:
    ...
    return liquid_glass_embed(
        "🛠️ Owner-Panel",
        f"**Server insgesamt:** {guild_count}\n**Mitglieder gesamt:** {member_count}\n**Ping:** {latency_ms} ms\n**Uptime:** {uptime_str}\n\nWähle unten einen Server aus, um ihn fernzusteuern.\n_(Nur die ersten 25 Server werden angezeigt.)_",
        discord.Color.from_rgb(140, 210, 255)
    )


def build_guild_overview_embed(guild: discord.Guild) -> discord.Embed:
    owner_mention = guild.owner.mention if guild.owner else "Unbekannt"
    created = guild.created_at.strftime("%d.%m.%Y")
    return liquid_glass_embed(
        f"🖥️ {guild.name}",
        f"**ID:** {guild.id}\n**Mitglieder:** {guild.member_count}\n**Owner:** {owner_mention}\n**Erstellt am:** {created}\n**Boost-Level:** {guild.premium_tier} ({guild.premium_subscription_count or 0} Boosts)\n\nWähle unten ein Setup-System aus, um es auf diesem Server einzurichten.",
        discord.Color.from_rgb(140, 210, 255)
    )


class PanelCategorySelect(discord.ui.Select):
    def __init__(self, invoker_id: int, guild: discord.Guild, specs: dict):
        self.invoker_id = invoker_id
        self.guild = guild
        self.specs = specs
        options = [discord.SelectOption(label=spec["label"][:100], value=key) for key, spec in specs.items()][:25]
        super().__init__(placeholder="Auswählen...", options=options, min_values=1, max_values=1)

    async def callback(self, interaction: discord.Interaction):
        if interaction.user.id != self.invoker_id:
            await interaction.response.send_message("❌ Dieses Panel ist nicht für dich.", ephemeral=True)
            return
        spec = self.specs[self.values[0]]
        session = SetupSession(self.guild, spec, self.invoker_id)
        await render_panel_step(interaction, session)


class PanelCategorySelectView(discord.ui.View):
    def __init__(self, invoker_id: int, guild: discord.Guild, specs: dict):
        super().__init__(timeout=None)
        self.invoker_id = invoker_id
        self.guild = guild
        self.add_item(PanelCategorySelect(invoker_id, guild, specs))
        back_btn = discord.ui.Button(label="⬅️ Zurück", style=discord.ButtonStyle.secondary)
    ...
    async def back(self, interaction: discord.Interaction):
        if interaction.user.id != self.invoker_id:
            await interaction.response.send_message("❌ Dieses Panel ist nicht für dich.", ephemeral=True)
            return
        embed = build_guild_overview_embed(self.guild)
        view = PanelAreaSelectView(self.invoker_id, self.guild)
        await safe_interaction_edit(interaction, embed=embed, view=view)


class PanelAreaSelect(discord.ui.Select):
    def __init__(self, invoker_id: int, guild: discord.Guild):
        self.invoker_id = invoker_id
        self.guild = guild
        options = [
            discord.SelectOption(label="⚙️ Setup-Systeme", value="setup", description="Kanäle/Rollen für Systeme einrichten"),
            discord.SelectOption(label="🛠️ Aktionen ausführen", value="action", description="Uprank, Teamwarn, XP usw. direkt ausführen"),
        ]
        super().__init__(placeholder="Bereich auswählen...", options=options, min_values=1, max_values=1)

    async def callback(self, interaction: discord.Interaction):
        if interaction.user.id != self.invoker_id:
            await interaction.response.send_message("❌ Dieses Panel ist nicht für dich.", ephemeral=True)
            return
        specs = SETUP_SPECS if self.values[0] == "setup" else ACTION_SPECS
        embed = build_guild_overview_embed(self.guild)
        view = PanelCategorySelectView(self.invoker_id, self.guild, specs)
        await safe_interaction_edit(interaction, embed=embed, view=view)


class PanelAreaSelectView(discord.ui.View):
    def __init__(self, invoker_id: int, guild: discord.Guild):
        super().__init__(timeout=None)
        self.invoker_id = invoker_id
        self.guild = guild
        self.add_item(PanelAreaSelect(invoker_id, guild))
        back_btn = discord.ui.Button(label="⬅️ Server wechseln", style=discord.ButtonStyle.secondary)
        back_btn.callback = self.back
        self.add_item(back_btn)
    ...
    async def back(self, interaction: discord.Interaction):
        if interaction.user.id != self.invoker_id:
            await interaction.response.send_message("❌ Dieses Panel ist nicht für dich.", ephemeral=True)
            return
        embed = build_panel_overview_embed()
        view = PanelGuildSelectView(self.invoker_id)
        await safe_interaction_edit(interaction, embed=embed, view=view)


class PanelGuildSelect(discord.ui.Select):
    def __init__(self, invoker_id: int):
        self.invoker_id = invoker_id
        guilds = list(bot.guilds)[:25]
        options = [
            discord.SelectOption(label=g.name[:100], value=str(g.id), description=f"{g.member_count or 0} Mitglieder • ID {g.id}"[:100])
            for g in guilds
        ] or [discord.SelectOption(label="Keine Server verfügbar", value="__none__")]
        super().__init__(placeholder="Server auswählen...", options=options, min_values=1, max_values=1)
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if self.values[0] == "__none__":
            await interaction.response.send_message("❌ Keine Server verfügbar.", ephemeral=True)
            return
        guild = bot.get_guild(int(self.values[0]))
        if not guild:
            await safe_interaction_edit(
                interaction,
                embed=liquid_glass_embed("❌ Fehler", "Server nicht gefunden.", discord.Color.from_rgb(220, 60, 60)),
                view=None
            )
            return
        embed = build_guild_overview_embed(guild)
        view = PanelAreaSelectView(self.invoker_id, guild)
        try:
            await interaction.response.edit_message(embed=embed, view=view)
        except discord.NotFound:
            # Interaction abgelaufen (z.B. Panel war zu lange offen) -> neue DM statt Crash
            try:
...
class PanelGuildSelectView(discord.ui.View):
    def __init__(self, invoker_id: int):
        super().__init__(timeout=None)
        self.invoker_id = invoker_id
        self.add_item(PanelGuildSelect(invoker_id))


@tree.command(name="panel", description="Owner-Panel: Bot fernsteuern (nur für den Bot-Owner)")
async def panel_cmd(interaction: discord.Interaction):
    if interaction.user.id != OWNER_ID:
        await interaction.response.send_message("❌ Dieses Panel ist nur für den Bot-Owner verfügbar.", ephemeral=True)
        return
    embed = build_panel_overview_embed()
    view = PanelGuildSelectView(interaction.user.id)
    try:
        await interaction.user.send(embed=embed, view=view)
        await interaction.response.send_message("📬 Ich hab dir das Owner-Panel per DM geschickt!", ephemeral=True)
    except discord.Forbidden:
        await interaction.response.send_message(
            "❌ Ich konnte dir keine DM schicken. Bitte aktiviere DMs von Servermitgliedern und versuche es erneut.",
            ephemeral=True
        )
