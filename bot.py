...
class TicketCloseActionView(discord.ui.View):
    ...
    async def _close(self, interaction: discord.Interaction, delete: bool):
        # Defer - falls schon geantwortet wurde (z.B. via on_interaction), ignorieren
        try:
            await interaction.response.defer(ephemeral=True)
        except Exception:
            pass
        ...
        # Ticket-Daten immer frisch aus DB laden (funktioniert auch nach Neustart)
        ...
        for key, t in tickets_db.get(guild_id, {}).items():
            if str(t.get("channel_id", "")) == channel_id_str or key == channel_id_str:
                ...
                self.ticket_key = key
                break
        # Fallback falls nicht in DB
        if not self.ticket_data:
            self.ticket_data = {"channel_id": channel_id_str, "user_id": 0, "category": "Unbekannt"}
        if not self.ticket_key:
            self.ticket_key = channel_id_str
        ...
        # followup.send kann fehlschlagen wenn die ephemeral Message nicht mehr existiert
        # → in try/except damit channel.delete() IMMER ausgeführt wird
        try:
        ...
        if delete:
            ...
            try:
                ...
                print(f"[TICKET] Kanal konnte nicht archiviert werden: {e}")
