...
class TicketNotifView(discord.ui.View):
    ...
    async def _accept(self, interaction: discord.Interaction, button: discord.ui.Button, channel_id: str):
        ...
        # Schließen-Button im Ticket-Channel selbst freischalten
        try:
            async for msg in ticket_channel.history(limit=20):
                if msg.author == bot.user and msg.components:
                    for row in msg.components:
                        for comp in row.children:
                            if getattr(comp, "custom_id", None) == "ticket_release":
    ...
    if notification_channel_id:
        try:
            ...
            if notif_channel:
                ...
                await notif_channel.send(content=ping_text if ping_role_id else None, embed=notif_embed, view=TicketNotifView(str(channel.id)))
        except Exception:
            pass
