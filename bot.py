...
class TicketView(discord.ui.View):
    def __init__(self, all_categories: dict = None, disabled_categories: list = None):
        super().__init__(timeout=None)
        disabled = disabled_categories or []
        base_cats = {k: (e, l, d, None) for k, (e, l, d) in TICKET_CATEGORIES.items() if k not in disabled}
        cats = all_categories if all_categories else base_cats
    ...
    if key in custom:
        ...
        await interaction.response.send_message(
            embed=liquid_glass_embed(
                ...
                f"Die custom Kategorie `{key}` wurde entfernt.",
                discord.Color.from_rgb(255, 100, 100)
            ),
            ephemeral=True
        )
    # Check standard categories
    elif key in TICKET_CATEGORIES:
        if key not in disabled:
            disabled.append(key)
        cfg["disabled_categories"] = disabled
        await save_ticket_config(interaction.guild.id, cfg)
        emoji, label, _ = TICKET_CATEGORIES[key]
        await interaction.response.send_message(
            embed=liquid_glass_embed(
                "🗑️ Standard-Kategorie deaktiviert",
                f"Die Kategorie **{emoji} {label}** (`{key}`) wurde deaktiviert.\n\nBenutze `/ticket-panel-aktualisieren` um das Panel zu aktualisieren!",
                discord.Color.from_rgb(255, 100, 100)
            ),
            ephemeral=True
        )
        ...
        # Show available keys
        std_keys = ", ".join(f"`{k}`" for k in TICKET_CATEGORIES.keys())
        custom_keys = ", ".join(f"`{k}`" for k in custom.keys()) if custom else "Keine"
        await interaction.response.send_message(
            f"Kategorie `{key}` nicht gefunden!\n\n**Standard:** {std_keys}\n**Custom:** {custom_keys}",
            ephemeral=True
        )
...
async def ticket_kategorien(interaction: discord.Interaction):
    ...
    disabled = cfg.get("disabled_categories", [])
    std_list = "\n".join(
        f"{e} **{l}** (`{k}`) {'~~deaktiviert~~' if k in disabled else ''}"
        for k, (e, l, _) in TICKET_CATEGORIES.items()
    )
