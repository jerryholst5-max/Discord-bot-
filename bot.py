...
class VoiceSupportCloseView(discord.ui.View):
    ...
    async def _close_callback(self, interaction: discord.Interaction):
        ...
        acceptor_text = f"\n**Support von:** {acceptor.mention}" if acceptor else ""

        await interaction.response.edit_message(
            embed=liquid_glass_embed(
                "✅ Support beendet",
                f"{member.mention if member else 'Der User'} wurde aus dem Support-Raum entfernt.{acceptor_text}{dauer_text}",
                discord.Color.from_rgb(130, 200, 240)
            ),
            view=self
        )
    ...
    for system_num, vs_cfg in [
        ...
        if warteraum_id and notif_channel_id:
            ...
            if after.channel and str(after.channel.id) == str(warteraum_id):
                ...
                break
            # Jemand verlässt den Warteraum (ohne angenommen zu werden)
                ...
                if notif_channel:
                    # Letzte Nachricht mit offenen Annehmen/Ablehnen Buttons für diesen User finden und updaten
                    try:
                        async for msg in notif_channel.history(limit=50):
                            if msg.author == guild.me and msg.embeds:
                                embed = msg.embeds[0]
                                # Prüfen ob Nachricht für diesen User ist und noch offen
                                if member.mention in (embed.description or "") and msg.components:
                                    for row in msg.components:
                                        for btn in row.children:
                                            if hasattr(btn, "custom_id") and btn.custom_id and btn.custom_id.startswith(f"vs_accept:{member.id}:"):
