...
async def update_partner_panel(guild: discord.Guild):
    """Aktualisiert das Partner-Panel im konfigurierten Kanal."""
    ...
    for p in partners:
        ...
        kategorien.setdefault(kat, []).append(p)
...
async def partner_name_autocomplete(interaction: discord.Interaction, current: str):
    ...
    return [
        ...
        for p in partners if current.lower() in p["name"].lower()
    ][:25]

async def partner_kat_autocomplete(interaction: discord.Interaction, current: str):
    cfg = await get_partner_config(interaction.guild.id)
    partners = cfg.get("partners", [])
    kats = list(set(p.get("kategorie", "🤝 Partner") for p in partners))
    return [
        app_commands.Choice(name=k, value=k)
        for k in kats if current.lower() in k.lower()
    ][:25]
...
async def partner_setup(interaction: discord.Interaction, kanal: discord.TextChannel):
    ...
    await interaction.followup.send(
        embed=liquid_glass_embed("✅ Partner-Panel eingerichtet", f"Das Panel wurde in {kanal.mention} erstellt.", discord.Color.from_rgb(100, 220, 150)),
        ephemeral=True
    )
...
@app_commands.describe(
    ...
    kategorie="Kategorie (z.B. 🌟 Große Partner)",
    ansprechpartner="Ansprechpartner Name (optional)",
    beschreibung="Kurze Beschreibung (optional)"
)
async def partner_hinzufuegen(
    interaction: discord.Interaction,
    name: str,
    link: str,
    kategorie: str = "🤝 Partner",
...
async def partner_entfernen(interaction: discord.Interaction, name: str):
    ...
    if len(new_partners) == len(partners):
        await interaction.followup.send(f"❌ Partner **{name}** nicht gefunden!", ephemeral=True)
        return
...
@app_commands.describe(
    ...
    neue_beschreibung="Neue Beschreibung (optional)"
)
@app_commands.autocomplete(name=partner_name_autocomplete)
async def partner_bearbeiten(
    interaction: discord.Interaction,
    name: str,
    neuer_name: str = None,
    neuer_link: str = None,
    neue_kategorie: str = None,
    neuer_ansprechpartner: str = None,
    neue_beschreibung: str = None
):
    if not interaction.user.guild_permissions.administrator:
        await interaction.response.send_message("❌ Keine Berechtigung!", ephemeral=True)
        return
    ...
    if neuer_link: partner["link"] = neuer_link; aenderungen.append(f"Link → **{neuer_link}**")
    ...
    if neue_beschreibung: partner["beschreibung"] = neue_beschreibung; aenderungen.append(f"Beschreibung aktualisiert")
    cfg["partners"] = partners
    await save_partner_config(interaction.guild.id, cfg)
    await update_partner_panel(interaction.guild)
    await interaction.followup.send(
        embed=liquid_glass_embed("✅ Partner bearbeitet", "\n".join(aenderungen) or "Keine Änderungen.", discord.Color.from_rgb(100, 180, 255)),
        ephemeral=True
    )
...
async def partner_liste(interaction: discord.Interaction):
    ...
    for i, p in enumerate(partners, 1):
        ...
        if not link.startswith("http"):
            link = f"https://{link}"
        lines.append(f"`{i}.` **[{p['name']}]({link})** • {p.get('kategorie','🤝')}")
    await interaction.followup.send(
        embed=liquid_glass_embed(f"🤝 Partnerliste ({len(partners)})", "\n".join(lines), discord.Color.from_rgb(88, 101, 242)),
        ephemeral=True
    )
...
async def partner_panel_aktualisieren(interaction: discord.Interaction):
    ...
    await update_partner_panel(interaction.guild)
    await interaction.followup.send(
        embed=liquid_glass_embed("✅ Panel aktualisiert", "Das Partner-Panel wurde neu gerendert.", discord.Color.from_rgb(100, 220, 150)),
        ephemeral=True
    )

@tree.command(name="partner-panel", description="Sendet das Partnerantragspanel in den konfigurierten Kanal (Admin)")
@app_commands.describe(kanal="Kanal wo das Bewerbungs-Panel gesendet wird")
async def partner_panel(interaction: discord.Interaction, kanal: discord.TextChannel):
    ...
    embed = discord.Embed(
        ...
        description=(
            "Du möchtest mit uns eine Partnerschaft eingehen?\n\n"
            "Klicke auf den Button unten um eine Bewerbung einzureichen!\n\n"
            "**Voraussetzungen:**\n"
            "• Mindestens 50 Mitglieder\n"
            "• Aktiver Server\n"
            "• Discord-Regelkonform"
        ),
        color=discord.Color.from_rgb(88, 101, 242)
    )
    ...
    await interaction.followup.send(
        embed=liquid_glass_embed("✅ Panel gesendet", f"Das Partnerantragspanel wurde in {kanal.mention} gesendet.", discord.Color.from_rgb(100, 220, 150)),
        ephemeral=True
    )

@tree.command(name="partner-tier-add", description="Fügt einen Partner-Tier hinzu (Admin)")
@app_commands.describe(
    name="Name des Tiers (z.B. Gold, Silber)",
    beschreibung="Beschreibung des Tiers",
    rolle="Rolle die diesem Tier zugewiesen wird (optional)"
)
async def partner_tier_add(interaction: discord.Interaction, name: str, beschreibung: str = "", rolle: discord.Role = None):
    ...
    await save_partner_config(interaction.guild.id, cfg)
    await interaction.followup.send(
        embed=liquid_glass_embed("✅ Tier hinzugefügt", f"Tier **{name}** wurde erstellt.", discord.Color.from_rgb(100, 220, 150)),
        ephemeral=True
    )
...
async def partner_tier_list(interaction: discord.Interaction):
    ...
    tiers = cfg.get("tiers", [])
    if not tiers:
        await interaction.followup.send("❌ Noch keine Tiers vorhanden. Nutze `/partner-tier-add`.", ephemeral=True)
        return
    lines = []
    for i, t in enumerate(tiers, 1):
        rolle_text = f" • <@&{t['rolle_id']}>" if t.get("rolle_id") else ""
        beschreibung = f"\n  ↳ {t['beschreibung']}" if t.get("beschreibung") else ""
        lines.append(f"`{i}.` **{t['name']}**{rolle_text}{beschreibung}")
    await interaction.followup.send(
        embed=liquid_glass_embed(f"📊 Partner-Tiers ({len(tiers)})", "\n".join(lines), discord.Color.from_rgb(88, 101, 242)),
        ephemeral=True
    )
...
async def partner_tier_autocomplete(interaction: discord.Interaction, current: str):
    cfg = await get_partner_config(interaction.guild.id)
    tiers = cfg.get("tiers", [])
    return [app_commands.Choice(name=t["name"], value=t["name"]) for t in tiers if current.lower() in t["name"].lower()][:25]

@tree.command(name="partner-tier-remove", description="Entfernt einen Partner-Tier (Admin)")
@app_commands.describe(name="Name des Tiers")
...
async def partner_tier_remove(interaction: discord.Interaction, name: str):
    ...
    await save_partner_config(interaction.guild.id, cfg)
    await interaction.followup.send(
        embed=liquid_glass_embed("✅ Tier entfernt", f"Tier **{name}** wurde entfernt.", discord.Color.from_rgb(220, 80, 80)),
        ephemeral=True
    )

@tree.command(name="partner-request", description="Starte eine Partnerschaftsanfrage")
@app_commands.describe(
    server_name="Name deines Servers",
    invite_link="Einladungslink deines Servers",
    beschreibung="Kurze Beschreibung deines Servers",
    mitglieder="Ungefähre Mitgliederzahl"
)
async def partner_request(interaction: discord.Interaction, server_name: str, invite_link: str, beschreibung: str, mitglieder: int):
    await interaction.response.defer(ephemeral=True)
    cfg = await get_partner_config(interaction.guild.id)
    bewerbung_channel_id = cfg.get("bewerbung_channel_id") or cfg.get("channel_id")
    if not bewerbung_channel_id:
        await interaction.followup.send("❌ Kein Bewerbungskanal eingerichtet. Admin muss `/partner-panel` ausführen.", ephemeral=True)
        return
    ch = interaction.guild.get_channel(int(bewerbung_channel_id))
    if not ch:
        await interaction.followup.send("❌ Bewerbungskanal nicht gefunden.", ephemeral=True)
        return
    embed = discord.Embed(
        title="📨 Neue Partnerschaftsbewerbung",
        color=discord.Color.from_rgb(88, 101, 242),
        timestamp=discord.utils.utcnow()
    )
    ...
    embed.set_footer(text=f"Bewerber ID: {interaction.user.id}")
    await ch.send(embed=embed)
    await interaction.followup.send(
        embed=liquid_glass_embed("✅ Bewerbung gesendet", "Deine Partnerschaftsbewerbung wurde erfolgreich eingereicht! Wir melden uns bald.", discord.Color.from_rgb(100, 220, 150)),
        ephemeral=True
    )

@tree.command(name="advertise", description="Richte dein Server-Werbeprofil im Partner-Netzwerk ein")
@app_commands.describe(
    server_name="Name deines Servers",
    invite_link="Einladungslink",
    beschreibung="Beschreibung deines Servers (max. 500 Zeichen)",
...
async def advertise(interaction: discord.Interaction, server_name: str, invite_link: str, beschreibung: str, banner_url: str = None):
    ...
    if len(beschreibung) > 500:
        await interaction.followup.send("❌ Beschreibung darf maximal 500 Zeichen lang sein!", ephemeral=True)
        return
    cfg = await get_partner_config(interaction.guild.id)
    advertise_channel_id = cfg.get("advertise_channel_id")
    if not advertise_channel_id:
        await interaction.followup.send("❌ Kein Werbekanal eingerichtet. Admin muss `/partner-setup` ausführen.", ephemeral=True)
        return
    ch = interaction.guild.get_channel(int(advertise_channel_id))
    if not ch:
        await interaction.followup.send("❌ Werbekanal nicht gefunden.", ephemeral=True)
        return
    ...
    embed.add_field(name="👤 Inseriert von", value=interaction.user.mention, inline=True)
    if banner_url:
        try:
            embed.set_image(url=banner_url)
        except Exception:
            pass
    embed.set_footer(text="GermanyRP • Partner-Netzwerk Werbung")
    await ch.send(embed=embed)
    await interaction.followup.send(
        embed=liquid_glass_embed("✅ Werbung gesendet", f"Dein Werbeprofil für **{server_name}** wurde im Partner-Netzwerk veröffentlicht!", discord.Color.from_rgb(100, 220, 150)),
        ephemeral=True
    )

# ── Partner Bewerbungs-View ──
...
class PartnerBewerbungModal(discord.ui.Modal, title="Partnerschaftsbewerbung"):
    ...
    beschreibung = discord.ui.TextInput(label="Beschreibung", style=discord.TextStyle.paragraph, placeholder="Kurze Beschreibung deines Servers...", required=True, max_length=500)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        await ch.send(embed=embed)
        await interaction.followup.send(
            embed=liquid_glass_embed("✅ Bewerbung gesendet!", "Deine Partnerschaftsbewerbung wurde eingereicht. Wir melden uns bald!", discord.Color.from_rgb(100, 220, 150)),
            ephemeral=True
        )
