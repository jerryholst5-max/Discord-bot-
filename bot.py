...
async def _twitch_refresh_user_token() -> bool:
    """Holt einen frischen Access Token über den Refresh Token. Gibt True bei Erfolg zurück."""
...
async def _twitch_irc_keepalive_loop():
    ...
    while True:
        try:
            ...
            line = await _twitch_irc_reader.readline()
            if not line:
                raise ConnectionError("IRC-Verbindung getrennt")
            text = line.decode(errors="ignore").strip()
            if text.startswith("PING"):
                _twitch_irc_writer.write("PONG :tmi.twitch.tv\r\n".encode())
                await _twitch_irc_writer.drain()
        except Exception as e:
            print(f"[TwitchChat] IRC-Fehler, verbinde neu in 10s: {e}")
            _twitch_irc_reader = None
            _twitch_irc_writer = None
            await asyncio.sleep(10)
...
async def _twitch_eventsub_loop():
    ...
    while True:
        try:
            ...
            print(f"[TwitchChat] EventSub-Verbindung getrennt, neuer Versuch in 15s: {e}")
...
@tree.command(name="teamwarn-setup", description="Legt fest welche Rollen bei 3 Warns entfernt werden (Admin)")
@app_commands.describe(
    von_rolle="Ab dieser Rolle werden Rollen entfernt (niedrigste)",
    bis_rolle="Bis zu dieser Rolle werden Rollen entfernt (höchste)"
)
async def teamwarn_setup(interaction: discord.Interaction, von_rolle: discord.Role, bis_rolle: discord.Role):
    if not has_permission(interaction, "administrator"):
        await interaction.response.send_message("Keine Berechtigung!", ephemeral=True)
        return
    await interaction.response.defer(ephemeral=True)
    cfg = await load_config()
    cfg.setdefault("teamwarn_config", {})[str(interaction.guild.id)] = {
    ...
    await interaction.followup.send(
        embed=liquid_glass_embed(
            "✅ Teamwarn Setup gespeichert",
            f"Bei **3 Verwarnungen** werden Rollen von **{von_rolle.mention}** bis **{bis_rolle.mention}** entfernt.",
            discord.Color.from_rgb(100, 220, 150)
        ),
        ephemeral=True
    )
...
async def teamwarn(interaction: discord.Interaction, member: discord.Member, grund: str = "Kein Grund"):
    ...
    # Frisch aus DB laden damit Cache-Probleme ausgeschlossen sind
    ...
    if count >= 3:
        try:
            # Rollen-Bereich aus Config laden
            cfg = await load_config()
            ...
            if von_rolle_id and bis_rolle_id:
                ...
                if von_rolle and bis_rolle:
                    # Nur Rollen im Bereich von_rolle.position bis bis_rolle.position entfernen
                ...
                # Kein Setup → alle Rollen entfernen
                roles_to_remove = [r for r in member.roles if r != interaction.guild.default_role and r.is_assignable()]

            if roles_to_remove:
                await member.remove_roles(*roles_to_remove, reason=f"Auto-Rollenentfernung nach {count} Verwarnungen")
            warnings_data[user_id] = []
            await save_warnings(warnings_data)
            rollen_text = ", ".join(r.mention for r in roles_to_remove) if roles_to_remove else "Keine Rollen entfernt"
    ...
    async def _close_callback(self, interaction: discord.Interaction):
        ...
        if member and member.voice and member.voice.channel:
            try:
                ...
                vs_cfg = await load_voice_support_config(guild.id)
                vs_cfg2 = await load_voice_support_config_2(guild.id)
...
async def partner_name_autocomplete(interaction: discord.Interaction, current: str):
    ...
    return [app_commands.Choice(name=p["name"], value=p["name"]) for p in cfg.get("partners", []) if current.lower() in p["name"].lower()][:25]

async def partner_tier_autocomplete(interaction: discord.Interaction, current: str):
    cfg = await get_partner_config(interaction.guild.id)
    return [app_commands.Choice(name=t["name"], value=t["name"]) for t in cfg.get("tiers", []) if current.lower() in t["name"].lower()][:25]
...
@app_commands.describe(name="Name", link="Einladungslink", kategorie="Kategorie", ansprechpartner="Ansprechpartner (optional)", beschreibung="Beschreibung (optional)")
async def partner_hinzufuegen(interaction: discord.Interaction, name: str, link: str, kategorie: str = "🤝 Partner", ansprechpartner: str = None, beschreibung: str = None):
    if not interaction.user.guild_permissions.administrator:
        await interaction.response.send_message("❌ Keine Berechtigung!", ephemeral=True)
        return
    await interaction.response.defer(ephemeral=True)
    ...
    if any(p["name"].lower() == name.lower() for p in partners):
        ...
        return
    from datetime import datetime as _dt
    partners.append({"name": name, "link": link, "kategorie": kategorie, "ansprechpartner": ansprechpartner or "", "beschreibung": beschreibung or "", "datum": _dt.now().strftime("%d.%m.%Y"), "hinzugefuegt_von": str(interaction.user)})
    cfg["partners"] = partners
    await save_partner_config(interaction.guild.id, cfg)
    await update_partner_panel(interaction.guild)
    await interaction.followup.send(embed=liquid_glass_embed("✅ Partner hinzugefügt", f"**{name}** → Kategorie **{kategorie}**", discord.Color.from_rgb(100, 220, 150)), ephemeral=True)
...
@app_commands.describe(name="Name", neuer_name="Neuer Name", neuer_link="Neuer Link", neue_kategorie="Neue Kategorie", neuer_ansprechpartner="Neuer Ansprechpartner", neue_beschreibung="Neue Beschreibung")
@app_commands.autocomplete(name=partner_name_autocomplete)
async def partner_bearbeiten(interaction: discord.Interaction, name: str, neuer_name: str = None, neuer_link: str = None, neue_kategorie: str = None, neuer_ansprechpartner: str = None, neue_beschreibung: str = None):
    if not interaction.user.guild_permissions.administrator:
        await interaction.response.send_message("❌ Keine Berechtigung!", ephemeral=True)
        return
    await interaction.response.defer(ephemeral=True)
    ...
    if neuer_link: partner["link"] = neuer_link; aenderungen.append(f"Link aktualisiert")
    ...
    await save_partner_config(interaction.guild.id, cfg)
    await update_partner_panel(interaction.guild)
    await interaction.followup.send(embed=liquid_glass_embed("✅ Partner bearbeitet", "\n".join(aenderungen) or "Keine Änderungen.", discord.Color.from_rgb(100, 180, 255)), ephemeral=True)
...
async def partner_info(interaction: discord.Interaction, name: str):
    ...
    await interaction.followup.send(embed=liquid_glass_embed(f"🤝 {partner['name']}", desc, discord.Color.from_rgb(88, 101, 242)), ephemeral=True)
...
async def partner_liste(interaction: discord.Interaction):
    ...
    if not partners:
        ...
        return
    lines = [f"`{i}.` **[{p['name']}]({'https://' + p['link'] if not p['link'].startswith('http') else p['link']})** • {p.get('kategorie','🤝')}" for i, p in enumerate(partners, 1)]
...
@tree.command(name="partner-panel-aktualisieren", description="Aktualisiert das Partner-Panel manuell (Admin)")
async def partner_panel_aktualisieren(interaction: discord.Interaction):
    if not interaction.user.guild_permissions.administrator:
        await interaction.response.send_message("❌ Keine Berechtigung!", ephemeral=True)
        return
    await interaction.response.defer(ephemeral=True)
    await update_partner_panel(interaction.guild)
    await interaction.followup.send(embed=liquid_glass_embed("✅ Panel aktualisiert", "Das Partner-Panel wurde neu gerendert.", discord.Color.from_rgb(100, 220, 150)), ephemeral=True)

@tree.command(name="partner-panel", description="Sendet das Partnerantragspanel in einen Kanal (Admin)")
@app_commands.describe(kanal="Kanal für Bewerbungen")
async def partner_panel(interaction: discord.Interaction, kanal: discord.TextChannel):
    if not interaction.user.guild_permissions.administrator:
        await interaction.response.send_message("❌ Keine Berechtigung!", ephemeral=True)
        return
    await interaction.response.defer(ephemeral=True)
    cfg = await get_partner_config(interaction.guild.id)
    cfg["bewerbung_channel_id"] = str(kanal.id)
    await save_partner_config(interaction.guild.id, cfg)
    embed = discord.Embed(title="🤝 Partnerschaftsbewerbung", description="Du möchtest mit uns eine Partnerschaft eingehen?\n\nKlicke auf den Button unten um eine Bewerbung einzureichen!", color=discord.Color.from_rgb(88, 101, 242))
    embed.set_footer(text=f"{interaction.guild.name} • Partnerschaft")
    await kanal.send(embed=embed, view=PartnerBewerbungView())
    await interaction.followup.send(embed=liquid_glass_embed("✅ Panel gesendet", f"Das Bewerbungs-Panel wurde in {kanal.mention} gesendet.", discord.Color.from_rgb(100, 220, 150)), ephemeral=True)

@tree.command(name="partner-tier-add", description="Fügt einen Partner-Tier hinzu (Admin)")
@app_commands.describe(name="Tier Name (z.B. Gold)", beschreibung="Beschreibung", rolle="Zugehörige Rolle (optional)")
async def partner_tier_add(interaction: discord.Interaction, name: str, beschreibung: str = "", rolle: discord.Role = None):
    if not interaction.user.guild_permissions.administrator:
        await interaction.response.send_message("❌ Keine Berechtigung!", ephemeral=True)
        return
    await interaction.response.defer(ephemeral=True)
    cfg = await get_partner_config(interaction.guild.id)
    tiers = cfg.get("tiers", [])
    if any(t["name"].lower() == name.lower() for t in tiers):
        await interaction.followup.send(f"❌ Tier **{name}** existiert bereits!", ephemeral=True)
        return
    tiers.append({"name": name, "beschreibung": beschreibung, "rolle_id": str(rolle.id) if rolle else ""})
    cfg["tiers"] = tiers
    await save_partner_config(interaction.guild.id, cfg)
    await interaction.followup.send(embed=liquid_glass_embed("✅ Tier hinzugefügt", f"Tier **{name}** wurde erstellt.", discord.Color.from_rgb(100, 220, 150)), ephemeral=True)

@tree.command(name="partner-tier-list", description="Zeigt alle Partner-Tiers an")
async def partner_tier_list(interaction: discord.Interaction):
    await interaction.response.defer(ephemeral=True)
    cfg = await get_partner_config(interaction.guild.id)
    tiers = cfg.get("tiers", [])
    if not tiers:
        await interaction.followup.send("❌ Noch keine Tiers. Nutze `/partner-tier-add`.", ephemeral=True)
        return
    lines = [f"`{i}.` **{t['name']}**{' • <@&' + t['rolle_id'] + '>' if t.get('rolle_id') else ''}{chr(10) + '  ↳ ' + t['beschreibung'] if t.get('beschreibung') else ''}" for i, t in enumerate(tiers, 1)]
    await interaction.followup.send(embed=liquid_glass_embed(f"📊 Partner-Tiers ({len(tiers)})", "\n".join(lines), discord.Color.from_rgb(88, 101, 242)), ephemeral=True)

@tree.command(name="partner-tier-remove", description="Entfernt einen Partner-Tier (Admin)")
@app_commands.describe(name="Tier Name")
@app_commands.autocomplete(name=partner_tier_autocomplete)
async def partner_tier_remove(interaction: discord.Interaction, name: str):
    if not interaction.user.guild_permissions.administrator:
        await interaction.response.send_message("❌ Keine Berechtigung!", ephemeral=True)
        return
    await interaction.response.defer(ephemeral=True)
    cfg = await get_partner_config(interaction.guild.id)
    tiers = cfg.get("tiers", [])
    new_tiers = [t for t in tiers if t["name"].lower() != name.lower()]
    if len(new_tiers) == len(tiers):
        await interaction.followup.send(f"❌ Tier **{name}** nicht gefunden!", ephemeral=True)
        return
    cfg["tiers"] = new_tiers
    await save_partner_config(interaction.guild.id, cfg)
    await interaction.followup.send(embed=liquid_glass_embed("✅ Tier entfernt", f"Tier **{name}** wurde entfernt.", discord.Color.from_rgb(220, 80, 80)), ephemeral=True)

@tree.command(name="partner-request", description="Starte eine Partnerschaftsanfrage")
@app_commands.describe(server_name="Name deines Servers", invite_link="Einladungslink", beschreibung="Kurze Beschreibung", mitglieder="Ungefähre Mitgliederzahl")
async def partner_request(interaction: discord.Interaction, server_name: str, invite_link: str, beschreibung: str, mitglieder: int):
    await interaction.response.defer(ephemeral=True)
    cfg = await get_partner_config(interaction.guild.id)
    ch_id = cfg.get("bewerbung_channel_id") or cfg.get("channel_id")
    if not ch_id:
        await interaction.followup.send("❌ Kein Bewerbungskanal eingerichtet.", ephemeral=True)
        return
    ch = interaction.guild.get_channel(int(ch_id))
    if not ch:
        await interaction.followup.send("❌ Kanal nicht gefunden.", ephemeral=True)
        return
    link = invite_link if invite_link.startswith("http") else f"https://{invite_link}"
    embed = discord.Embed(title="📨 Neue Partnerschaftsbewerbung", color=discord.Color.from_rgb(88, 101, 242), timestamp=discord.utils.utcnow())
    embed.add_field(name="👤 Bewerber", value=interaction.user.mention, inline=True)
    embed.add_field(name="🏠 Server", value=server_name, inline=True)
    embed.add_field(name="👥 Mitglieder", value=str(mitglieder), inline=True)
    embed.add_field(name="🔗 Invite", value=link, inline=False)
    embed.add_field(name="📝 Beschreibung", value=beschreibung, inline=False)
    embed.set_footer(text=f"ID: {interaction.user.id}")
    await ch.send(embed=embed)
    await interaction.followup.send(embed=liquid_glass_embed("✅ Bewerbung gesendet", "Deine Partnerschaftsbewerbung wurde eingereicht!", discord.Color.from_rgb(100, 220, 150)), ephemeral=True)

@tree.command(name="advertise", description="Veröffentliche dein Server-Werbeprofil")
@app_commands.describe(server_name="Server Name", invite_link="Einladungslink", beschreibung="Beschreibung (max. 500 Zeichen)", banner_url="Banner URL (optional)")
async def advertise(interaction: discord.Interaction, server_name: str, invite_link: str, beschreibung: str, banner_url: str = None):
    await interaction.response.defer(ephemeral=True)
    if len(beschreibung) > 500:
        await interaction.followup.send("❌ Max. 500 Zeichen!", ephemeral=True)
        return
    cfg = await get_partner_config(interaction.guild.id)
    ch_id = cfg.get("advertise_channel_id") or cfg.get("channel_id")
    if not ch_id:
        await interaction.followup.send("❌ Kein Werbekanal eingerichtet.", ephemeral=True)
        return
    ch = interaction.guild.get_channel(int(ch_id))
    if not ch:
        await interaction.followup.send("❌ Kanal nicht gefunden.", ephemeral=True)
        return
    link = invite_link if invite_link.startswith("http") else f"https://{invite_link}"
    embed = discord.Embed(title=f"📢 {server_name}", description=beschreibung, color=discord.Color.from_rgb(255, 165, 0), timestamp=discord.utils.utcnow())
    embed.add_field(name="🔗 Einladung", value=f"[Beitreten]({link})", inline=True)
    embed.add_field(name="👤 Von", value=interaction.user.mention, inline=True)
    if banner_url:
        try: embed.set_image(url=banner_url)
        except Exception: pass
    embed.set_footer(text="GermanyRP • Partner-Netzwerk")
    await ch.send(embed=embed)
    await interaction.followup.send(embed=liquid_glass_embed("✅ Werbung veröffentlicht", f"**{server_name}** ist jetzt im Partner-Netzwerk sichtbar!", discord.Color.from_rgb(100, 220, 150)), ephemeral=True)

class PartnerBewerbungModal(discord.ui.Modal, title="Partnerschaftsbewerbung"):
    server_name = discord.ui.TextInput(label="Server Name", placeholder="Name deines Servers", required=True)
    invite_link = discord.ui.TextInput(label="Einladungslink", placeholder="discord.gg/example", required=True)
    mitglieder = discord.ui.TextInput(label="Mitgliederzahl", placeholder="z.B. 500", required=True)
    beschreibung = discord.ui.TextInput(label="Beschreibung", style=discord.TextStyle.paragraph, placeholder="Kurze Beschreibung...", required=True, max_length=500)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        ch_id = cfg.get("bewerbung_channel_id") or cfg.get("channel_id")
        if not ch_id:
            await interaction.followup.send("❌ Kein Bewerbungskanal konfiguriert.", ephemeral=True)
            return
        ch = interaction.guild.get_channel(int(ch_id))
        if not ch:
            await interaction.followup.send("❌ Kanal nicht gefunden.", ephemeral=True)
            return
        link = str(self.invite_link.value)
        if not link.startswith("http"): link = f"https://{link}"
        embed = discord.Embed(title="📨 Neue Partnerschaftsbewerbung", color=discord.Color.from_rgb(88, 101, 242), timestamp=discord.utils.utcnow())
        embed.add_field(name="👤 Bewerber", value=interaction.user.mention, inline=True)
        embed.add_field(name="🏠 Server", value=str(self.server_name.value), inline=True)
        embed.add_field(name="👥 Mitglieder", value=str(self.mitglieder.value), inline=True)
        embed.add_field(name="🔗 Invite", value=link, inline=False)
        embed.add_field(name="📝 Beschreibung", value=str(self.beschreibung.value), inline=False)
        embed.set_footer(text=f"ID: {interaction.user.id}")
        await ch.send(embed=embed)
        await interaction.followup.send(embed=liquid_glass_embed("✅ Bewerbung gesendet!", "Wir melden uns bald!", discord.Color.from_rgb(100, 220, 150)), ephemeral=True)

class PartnerBewerbungView(discord.ui.View):
    def __init__(self):
        super().__init__(timeout=None)

    @discord.ui.button(label="🤝 Jetzt bewerben", style=discord.ButtonStyle.primary, custom_id="partner_bewerben")
    async def bewerben(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_modal(PartnerBewerbungModal())
