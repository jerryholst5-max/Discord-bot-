...
# user_id -> Captcha-Code (kurzlebig, nur im Speicher – kein DB-Bedarf für ein paar Minuten Gültigkeit)
_pending_captchas: dict[int, str] = {}

def _generate_captcha_image() -> tuple[str, discord.File]:
    """Erstellt ein Captcha-Bild mit zufälligem Code und gibt (Code, discord.File) zurück."""
    zeichen = "ABCDEFGHJKLMNPQRSTUVWXYZ23456789"  # verwechselbare Zeichen (0/O, 1/I/l) ausgeschlossen
    code = "".join(random.choices(zeichen, k=6))

    breite, hoehe = 1000, 350
    bild = Image.new("RGB", (breite, hoehe), color=(25, 25, 30))
    draw = ImageDraw.Draw(bild)

    # Rausch-Linien im Hintergrund (dezent, damit die Zeichen klar lesbar bleiben)
    for _ in range(5):
        start = (random.randint(0, breite), random.randint(0, hoehe))
        ende = (random.randint(0, breite), random.randint(0, hoehe))
        draw.line([start, ende], fill=(random.randint(50, 80), random.randint(50, 80), random.randint(50, 80)), width=4)

    try:
        schrift = ImageFont.truetype("/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf", 176)
    except Exception:
        schrift = ImageFont.load_default()

    x = 48
    for zeichen_einzel in code:
        y = random.randint(40, 96)
        winkel = random.randint(-15, 15)
        zeichen_bild = Image.new("RGBA", (208, 240), (0, 0, 0, 0))
        zeichen_draw = ImageDraw.Draw(zeichen_bild)
        farbe = (255, random.randint(200, 230), random.randint(40, 70))
        zeichen_draw.text((16, 8), zeichen_einzel, font=schrift, fill=farbe)
        zeichen_bild = zeichen_bild.rotate(winkel, expand=1)
        bild.paste(zeichen_bild, (x, y), zeichen_bild)
        x += 147

    # Ein paar wenige Linien vor den Zeichen, aber dünner und nur 3 Stück
    for _ in range(3):
        start = (random.randint(0, breite), random.randint(0, hoehe))
        ende = (random.randint(0, breite), random.randint(0, hoehe))
        draw.line([start, ende], fill=(random.randint(70, 110), random.randint(70, 110), random.randint(70, 110)), width=3)

    # Rausch-Punkte (leicht reduziert)
    for _ in range(500):
        px, py = random.randint(0, breite - 1), random.randint(0, hoehe - 1)
        draw.point((px, py), fill=(random.randint(80, 140), random.randint(80, 140), random.randint(80, 140)))

    buffer = io.BytesIO()
    bild.save(buffer, format="PNG")
    buffer.seek(0)
    return code, discord.File(buffer, filename="captcha.png")


class CaptchaModal(discord.ui.Modal, title="Captcha bestätigen"):
    code = discord.ui.TextInput(label="Gib den Code aus dem Bild ein", max_length=10)

    async def on_submit(self, interaction: discord.Interaction):
        richtiger_code = _pending_captchas.get(interaction.user.id)
        if not richtiger_code:
            await interaction.response.send_message("❌ Dein Captcha ist abgelaufen. Klicke erneut auf „Verifizieren“, um ein neues zu bekommen.", ephemeral=True)
            return
        if str(self.code.value).strip().upper() != richtiger_code:
            await interaction.response.send_message("❌ Falscher Code! Klicke erneut auf „Verifizieren“, um es nochmal zu versuchen.", ephemeral=True)
            _pending_captchas.pop(interaction.user.id, None)
            return

        _pending_captchas.pop(interaction.user.id, None)
        cfg = await get_verify_config(interaction.guild.id)
        anfrage_kanal_id = cfg.get("anfrage_channel_id")
        pruefer_rolle_id = cfg.get("pruefer_role_id")
        anfrage_kanal = interaction.guild.get_channel(int(anfrage_kanal_id)) if anfrage_kanal_id else None
        if not anfrage_kanal or not pruefer_rolle_id:
        ...
        await interaction.response.send_message("✅ Captcha korrekt! Deine Verifizierungs-Anfrage wurde gesendet. Bitte warte, bis das Team sie bestätigt.", ephemeral=True)

        embed = liquid_glass_embed(
            "📨 Neue Verifizierungs-Anfrage",
            f"{interaction.user.mention} (`{interaction.user.id}`) hat das Captcha gelöst und möchte sich verifizieren.",
            discord.Color.from_rgb(240, 165, 0)
        )
        embed.set_thumbnail(url=interaction.user.display_avatar.url)
        await anfrage_kanal.send(content=f"<@&{pruefer_rolle_id}>", embed=embed, view=VerifyAnfrageView(str(interaction.user.id)))


class CaptchaLoesenView(discord.ui.View):
    def __init__(self):
        super().__init__(timeout=180)

    @discord.ui.button(label="Code eingeben", style=discord.ButtonStyle.primary, custom_id="verify_captcha_eingeben")
    async def eingeben(self, interaction: discord.Interaction, button: discord.ui.Button):
...
class VerifyButtonView(discord.ui.View):
    ...
    async def verifizieren(self, interaction: discord.Interaction, button: discord.ui.Button):
        cfg = await get_verify_config(interaction.guild.id)
        rollen_ids = cfg.get("verified_role_ids", [])
        anfrage_kanal_id = cfg.get("anfrage_channel_id")
        pruefer_rolle_id = cfg.get("pruefer_role_id")
        if not rollen_ids or not anfrage_kanal_id or not pruefer_rolle_id:
            await interaction.response.send_message("❌ Das Verifizierungssystem ist nicht vollständig eingerichtet. Bitte wende dich an das Team.", ephemeral=True)
            return
        erste_rolle = interaction.guild.get_role(int(rollen_ids[0]))
        anfrage_kanal = interaction.guild.get_channel(int(anfrage_kanal_id))
        if not erste_rolle or not anfrage_kanal:
            await interaction.response.send_message("❌ Die Konfiguration wurde nicht gefunden. Bitte wende dich an das Team.", ephemeral=True)
            return
        if erste_rolle in interaction.user.roles:
            await interaction.response.send_message("✅ Du bist bereits verifiziert!", ephemeral=True)
            return

        code, bild_datei = _generate_captcha_image()
        _pending_captchas[interaction.user.id] = code
        await interaction.response.send_message(
            "🔐 Bitte löse das Captcha, um deine Verifizierungs-Anfrage zu senden. Der Code ist 3 Minuten gültig.",
            file=bild_datei,
            view=CaptchaLoesenView(),
            ephemeral=True
        )


class VerifyAnfrageView(discord.ui.View):
    def __init__(self, antragsteller_id: str):
        super().__init__(timeout=None)
        self.antragsteller_id = antragsteller_id

    @discord.ui.button(label="✅ Annehmen", style=discord.ButtonStyle.success, custom_id="verify_anfrage_annehmen")
    async def annehmen(self, interaction: discord.Interaction, button: discord.ui.Button):
        cfg = await get_verify_config(interaction.guild.id)
        pruefer_rolle_id = cfg.get("pruefer_role_id")
        if pruefer_rolle_id and not any(str(r.id) == pruefer_rolle_id for r in interaction.user.roles) and not is_bot_owner(interaction.user):
            await interaction.response.send_message("❌ Keine Berechtigung, Verifizierungen zu bestätigen!", ephemeral=True)
            return

        rollen_ids = cfg.get("verified_role_ids", [])
        rollen = [interaction.guild.get_role(int(rid)) for rid in rollen_ids]
        rollen = [r for r in rollen if r is not None]
        antragsteller = interaction.guild.get_member(int(self.antragsteller_id))
        if not rollen or not antragsteller:
            await interaction.response.send_message("❌ Nutzer oder Rollen nicht mehr gefunden.", ephemeral=True)
            return
        try:
            await antragsteller.add_roles(*rollen, reason=f"Verifizierung bestätigt von {interaction.user}")
        except discord.Forbidden:
            await interaction.response.send_message("❌ Mir fehlt die Berechtigung, die Rollen zu vergeben.", ephemeral=True)
            return

        embed = interaction.message.embeds[0]
        embed.title = "✅ Verifizierung bestätigt"
        embed.color = discord.Color.from_rgb(100, 220, 150)
        ...
        if log_channel_id:
            ...
            if log_channel:
                try:
                    await log_channel.send(embed=liquid_glass_embed("✅ Neue Verifizierung", f"{antragsteller.mention} (`{antragsteller.id}`) wurde von {interaction.user.mention} verifiziert.", discord.Color.from_rgb(100, 220, 150)))
                except Exception:
                    pass

    @discord.ui.button(label="❌ Ablehnen", style=discord.ButtonStyle.danger, custom_id="verify_anfrage_ablehnen")
    async def ablehnen(self, interaction: discord.Interaction, button: discord.ui.Button):
        cfg = await get_verify_config(interaction.guild.id)
        pruefer_rolle_id = cfg.get("pruefer_role_id")
        if pruefer_rolle_id and not any(str(r.id) == pruefer_rolle_id for r in interaction.user.roles) and not is_bot_owner(interaction.user):
            await interaction.response.send_message("❌ Keine Berechtigung, Verifizierungen abzulehnen!", ephemeral=True)
            return

        embed = interaction.message.embeds[0]
        embed.title = "❌ Verifizierung abgelehnt"
        embed.color = discord.Color.from_rgb(220, 80, 80)
        embed.add_field(name="Abgelehnt von", value=interaction.user.mention, inline=True)
        await interaction.response.edit_message(embed=embed, view=None)
        ...
        if antragsteller:
            try:
                await antragsteller.send("❌ Deine Verifizierungs-Anfrage wurde leider abgelehnt. Bei Fragen wende dich an das Team.")
            except Exception:
                pass
...
@app_commands.describe(
    ...
    rolle1="Erste Rolle, die man nach der Verifizierung erhält",
    anfrage_kanal="Kanal, in dem Verifizierungs-Anfragen zur Bestätigung gepostet werden",
    pruefer_rolle="Rolle, die Verifizierungs-Anfragen annehmen/ablehnen darf",
    rolle2="Zweite Rolle (optional)", rolle3="Dritte Rolle (optional)",
    rolle4="Vierte Rolle (optional)", rolle5="Fünfte Rolle (optional)",
    log_kanal="Kanal für Verifizierungs-Logs (optional)"
)
async def verify_setup(interaction: discord.Interaction, verify_kanal: discord.TextChannel, rolle1: discord.Role, anfrage_kanal: discord.TextChannel, pruefer_rolle: discord.Role, rolle2: discord.Role = None, rolle3: discord.Role = None, rolle4: discord.Role = None, rolle5: discord.Role = None, log_kanal: discord.TextChannel = None):
    if not interaction.user.guild_permissions.administrator:
        await interaction.response.send_message("❌ Keine Berechtigung!", ephemeral=True)
        return
    ...
    await interaction.response.defer(ephemeral=True)
    cfg = await get_verify_config(interaction.guild.id)
    cfg["verified_role_ids"] = [str(r.id) for r in rollen]
    cfg["verify_channel_id"] = str(verify_kanal.id)
    cfg["anfrage_channel_id"] = str(anfrage_kanal.id)
    cfg["pruefer_role_id"] = str(pruefer_rolle.id)
    if log_kanal:
    ...
    embed = liquid_glass_embed(
        "✅ Verifizierung",
        "Willkommen! Klicke auf den Button unten, um eine Verifizierungs-Anfrage zu senden.",
        discord.Color.from_rgb(100, 220, 150)
    )
    await verify_kanal.send(embed=embed, view=VerifyButtonView())
    await interaction.followup.send(embed=liquid_glass_embed(
        "✅ Verifizierungssystem eingerichtet",
        f"Panel gepostet in {verify_kanal.mention}\nRollen: {', '.join(r.mention for r in rollen)}\nAnfragen gehen an: {anfrage_kanal.mention}\nBestätigen dürfen: {pruefer_rolle.mention}" + (f"\nLog-Kanal: {log_kanal.mention}" if log_kanal else ""),
        discord.Color.from_rgb(100, 220, 150)
    ), ephemeral=True)
