...
async def on_member_update(before: discord.Member, after: discord.Member):
    ...
    if before.roles != after.roles:
        ...
        # Executor ermitteln (wer hat die Rolle geändert)
        executor = None
        try:
...
@tree.command(name="ranklog-setup", description="Richtet den Ranklog-Kanal ein (Admin)")
@app_commands.describe(kanal="Kanal wo Rang-Änderungen geloggt werden")
async def ranklog_setup(interaction: discord.Interaction, kanal: discord.TextChannel):
    if not interaction.user.guild_permissions.administrator:
        await interaction.response.send_message("Keine Berechtigung!", ephemeral=True)
        return
    await interaction.response.defer(ephemeral=True)
    await save_ranklog_config(interaction.guild.id, {"channel_id": str(kanal.id)})
    await interaction.followup.send(
        embed=liquid_glass_embed(
            "✅ Ranklog eingerichtet!",
            f"Rang-Änderungen werden ab jetzt in {kanal.mention} geloggt.\n\n📈 Hochstufungen und 📉 Rückstufungen werden automatisch erfasst.",
            discord.Color.from_rgb(100, 220, 150)
        ),
        ephemeral=True
    )
