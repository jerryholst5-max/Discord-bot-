...
async def get_audit_executor_and_reason(guild, action):
    """Wie get_audit_executor, liefert aber zusätzlich den im Audit-Log hinterlegten Grund."""
    try:
        async for entry in guild.audit_logs(limit=1, action=action):
            if (datetime.now(timezone.utc) - entry.created_at).total_seconds() < 5:
...
LOG_CATEGORIES = {
    ...
    "server": "🛠️ Server-Änderungen (Rollen/Kanäle)",
    "voice": "🎙️ Voice-Aktivität (Join/Leave/Switch)",
    "members": "👤 Mitglieder (Join/Leave/Nickname)",
}

# Akzentfarbe + großes Banner-Icon pro Kategorie für den Premium-Look
LOG_CATEGORY_STYLE = {
    "moderation": (discord.Color.from_rgb(255, 90, 90), "🔨"),
    "messages":   (discord.Color.from_rgb(255, 170, 60), "📝"),
    "server":     (discord.Color.from_rgb(140, 180, 255), "🛠️"),
    "voice":      (discord.Color.from_rgb(160, 110, 255), "🎙️"),
    "members":    (discord.Color.from_rgb(100, 220, 170), "👤"),
}

async def get_unified_log_config(guild_id: int) -> dict:
    try:
        db = get_db()
        col = db["unified_log_config"]
        doc = await col.find_one({"guild_id": str(guild_id)})
        return doc or {}
    except Exception:
        return {}

async def save_unified_log_config(guild_id: int, data: dict):
    try:
        db = get_db()
        col = db["unified_log_config"]
...
async def send_unified_log(guild: discord.Guild, category: str, embed: discord.Embed):
    """Sendet einen Log-Eintrag im Premium Liquid-Glass Look wenn die Kategorie aktiviert ist."""
    cfg = await get_unified_log_config(guild.id)
    if not cfg.get("channel_id"):
        return
    enabled_categories = cfg.get("categories", [])
    ...
    # Themen-Icons pro Feldname statt generischer Bullets
    FIELD_ICONS = {
        "user": "👤", "moderator": "🛡️", "kanal": "📡", "grund": "📋",
        "dauer": "⏱️", "von": "↪️", "nach": "↩️", "vorher": "🔙", "nachher": "🆕",
        "rolle": "🎭", "verwarnung nr.": "🔢", "mitglieder jetzt": "📊",
        "account erstellt": "📅", "hinzugefügt": "✅", "entfernt": "🚫",
        "inhalt": "💬",
    }
    DEFAULT_FIELD_ICON = "▸"

    # Schweregrad-Badge pro Kategorie
    SEVERITY = {
        "moderation": "🔴 HOCH",
        "messages":   "🟡 MITTEL",
        "server":     "🟠 MITTEL",
    ...
    color, icon = LOG_CATEGORY_STYLE.get(category, (discord.Color.from_rgb(140, 210, 255), "💠"))
    raw_title = embed.title
    for _, ic in LOG_CATEGORY_STYLE.values():
        if raw_title.startswith(ic):
            raw_title = raw_title[len(ic):].strip()
            break

    cat_label = LOG_CATEGORIES.get(category, "Log").split(" ", 1)[-1]
    ...
    banner = (
        "```ansi\n"
        f"╔═══════════════════════════════════════╗\n"
        f"   {icon}  {raw_title.upper()}\n"
        f"   PRIORITÄT: {severity}\n"
        f"╚═══════════════════════════════════════╝\n"
        "```"
    )
    ...
    for f in embed.fields:
        clean_name = f.name.lstrip("▸◈✦❖ ").strip()
        field_icon = FIELD_ICONS.get(clean_name.lower(), DEFAULT_FIELD_ICON)
        styled.add_field(name=f"{field_icon}　**{clean_name.upper()}**", value=f.value, inline=f.inline)

    if embed.thumbnail:
        styled.set_thumbnail(url=embed.thumbnail.url)
    elif guild.icon:
        styled.set_thumbnail(url=guild.icon.url)

    styled.set_footer(
        text=f"◆ GermanyRP System  ◆  {cat_label}  ◆  {now_str} UTC",
...
@tree.command(name="logs-setup", description="Richtet das zentrale Log-System ein (Admin)")
@app_commands.describe(
    kanal="Kanal in den ALLE Logs gesendet werden",
    moderation="Ban/Kick/Mute/Warn loggen",
    nachrichten="Gelöschte/bearbeitete Nachrichten loggen",
    server_aenderungen="Rollen/Kanäle erstellt/gelöscht loggen",
    voice="Voice Join/Leave/Switch loggen",
    mitglieder="Mitglied beigetreten/verlassen/Nickname geändert loggen"
)
async def logs_setup(
    interaction: discord.Interaction,
    kanal: discord.TextChannel,
    moderation: bool = True,
    nachrichten: bool = True,
    server_aenderungen: bool = True,
    voice: bool = True,
    mitglieder: bool = True
):
    ...
    categories = []
    if moderation: categories.append("moderation")
    if nachrichten: categories.append("messages")
    if server_aenderungen: categories.append("server")
    if voice: categories.append("voice")
    if mitglieder: categories.append("members")

    await save_unified_log_config(interaction.guild.id, {
        "channel_id": str(kanal.id),
        "categories": categories
    })

    aktiv_text = "\n".join([f"✅ {LOG_CATEGORIES[c]}" for c in categories]) or "Keine Kategorien aktiv."
    deaktiv = [c for c in LOG_CATEGORIES if c not in categories]
    if deaktiv:
        aktiv_text += "\n" + "\n".join([f"❌ {LOG_CATEGORIES[c]}" for c in deaktiv])

    await interaction.followup.send(
        embed=liquid_glass_embed(
            "✅ Log-System eingerichtet",
            f"**Kanal:** {kanal.mention}\n\n{aktiv_text}\n\n💡 Führe `/logs-setup` erneut aus um Kategorien zu ändern.",
...
async def on_member_ban(guild, user):
    ...
    embed.add_field(name="Moderator", value=f"{executor.mention} ({executor})" if executor else "*Unbekannt*", inline=True)
    embed.add_field(name="Grund", value=ban_reason or "*Kein Grund angegeben*", inline=False)
    embed.set_thumbnail(url=user.display_avatar.url)
    await send_unified_log(guild, "moderation", embed)

@bot.event
async def on_member_remove(member):
    # Sofortiger Check: falls der Bot-Owner selbst den Server verlässt/gekickt/gebannt wird
    if member.id == OWNER_ID:
        await check_and_leave_if_owner_missing(member.guild)
        return

    executor, kick_reason = await get_audit_executor_and_reason(member.guild, discord.AuditLogAction.kick)
    if executor:
    ...
    # Unified Log
    if executor:
        log_embed = discord.Embed(title="👢 Mitglied gekickt", color=discord.Color.from_rgb(255, 150, 50), timestamp=datetime.now(timezone.utc))
        log_embed.add_field(name="User", value=f"**{member}**\n`{member.id}`", inline=True)
        log_embed.add_field(name="Moderator", value=f"{executor.mention} ({executor})", inline=True)
        log_embed.add_field(name="Grund", value=kick_reason or "*Kein Grund angegeben*", inline=False)
        log_embed.set_thumbnail(url=member.display_avatar.url)
        await send_unified_log(guild, "moderation", log_embed)
    else:
        log_embed = discord.Embed(title="👋 Mitglied verlassen", color=discord.Color.from_rgb(220, 80, 80), timestamp=datetime.now(timezone.utc))
        log_embed.add_field(name="User", value=f"**{member}**\n`{member.id}`", inline=True)
        log_embed.add_field(name="Mitglieder jetzt", value=str(guild.member_count), inline=True)
        log_embed.set_thumbnail(url=member.display_avatar.url)
        await send_unified_log(guild, "members", log_embed)
    ...
    # Teamliste aktualisieren falls der User eine Teamrolle hatte
    try:
        tl_cfg = await get_teamliste_config(guild.id)
        if tl_cfg:
        ...
        print(f"[Teamliste] Fehler beim Aktualisieren nach Mitglied-Verlassen: {e}")
...
async def on_guild_channel_delete(channel):
    ...
    embed = discord.Embed(title="🗑️ Kanal gelöscht", color=discord.Color.from_rgb(220, 80, 80), timestamp=datetime.now(timezone.utc))
    embed.add_field(name="Kanal", value=f"#{channel.name}", inline=True)
    embed.add_field(name="Von", value=str(executor) if executor else "Unbekannt", inline=True)
    await send_unified_log(channel.guild, "server", embed)

@bot.event
async def on_guild_channel_create(channel):
    executor = await get_audit_executor(channel.guild, discord.AuditLogAction.channel_create)
    embed = discord.Embed(title="➕ Kanal erstellt", color=discord.Color.from_rgb(100, 220, 150), timestamp=datetime.now(timezone.utc))
    embed.add_field(name="Kanal", value=channel.mention if hasattr(channel, "mention") else f"#{channel.name}", inline=True)
    embed.add_field(name="Von", value=str(executor) if executor else "Unbekannt", inline=True)
    await send_unified_log(channel.guild, "server", embed)
...
async def on_guild_role_delete(role):
    ...
    embed = discord.Embed(title="🗑️ Rolle gelöscht", color=discord.Color.from_rgb(220, 80, 80), timestamp=datetime.now(timezone.utc))
    embed.add_field(name="Rolle", value=role.name, inline=True)
    embed.add_field(name="Von", value=str(executor) if executor else "Unbekannt", inline=True)
    await send_unified_log(role.guild, "server", embed)

@bot.event
async def on_guild_role_create(role):
    executor = await get_audit_executor(role.guild, discord.AuditLogAction.role_create)
    embed = discord.Embed(title="➕ Rolle erstellt", color=discord.Color.from_rgb(100, 220, 150), timestamp=datetime.now(timezone.utc))
    embed.add_field(name="Rolle", value=role.mention, inline=True)
    embed.add_field(name="Von", value=str(executor) if executor else "Unbekannt", inline=True)
    await send_unified_log(role.guild, "server", embed)

# ─────────────────────────────────────────────
# OWNER-SCHUTZ: Bot verlässt Server automatisch
# wenn der Owner (OWNER_ID) dort nicht Mitglied ist
# ─────────────────────────────────────────────

async def check_and_leave_if_owner_missing(guild: discord.Guild):
    """Verlässt den Server, falls der Bot-Owner dort kein Mitglied ist."""
    try:
        ...
        if member is None:
            try:
                ...
                member = None
        if member is None:
            print(f"[Owner-Schutz] Owner nicht auf '{guild.name}' ({guild.id}) gefunden – verlasse Server.")
            try:
                await guild.leave()
            except Exception as e:
                print(f"[Owner-Schutz] Konnte Server '{guild.name}' nicht verlassen: {e}")
    except Exception as e:
        print(f"[Owner-Schutz] Fehler bei Prüfung für '{guild.name}': {e}")

async def owner_protection_loop():
    """Prüft periodisch alle Server – verlässt jeden, auf dem der Owner nicht mehr Mitglied ist."""
    await bot.wait_until_ready()
    while not bot.is_closed():
        try:
            for guild in list(bot.guilds):
                await check_and_leave_if_owner_missing(guild)
...
async def on_guild_join(guild: discord.Guild):
    """Wird aufgerufen sobald der Bot zu einem neuen Server hinzugefügt wird."""
    await check_and_leave_if_owner_missing(guild)
...
async def on_ready():
    ...
    for guild in list(bot.guilds):
        asyncio.ensure_future(check_and_leave_if_owner_missing(guild))
...
async def tempmute(interaction: discord.Interaction, member: discord.Member, minuten: int, grund: str = "Kein Grund"):
    ...
    try:
        ...
        await interaction.response.send_message(embed=embed)
        log_embed = discord.Embed(title="🔇 User stummgeschaltet", color=discord.Color.from_rgb(200, 100, 240), timestamp=datetime.now(timezone.utc))
        log_embed.add_field(name="User", value=member.mention, inline=True)
        log_embed.add_field(name="Dauer", value=f"{minuten} Minuten", inline=True)
        ...
        await send_unified_log(interaction.guild, "moderation", log_embed)
    except discord.Forbidden:
        await interaction.response.send_message("Fehlende Berechtigung.", ephemeral=True)
...
async def unmute(interaction: discord.Interaction, member: discord.Member):
    ...
    try:
        ...
        await interaction.response.send_message(embed=embed)
        log_embed = discord.Embed(title="🔊 Mute aufgehoben", color=discord.Color.from_rgb(100, 220, 150), timestamp=datetime.now(timezone.utc))
        log_embed.add_field(name="User", value=member.mention, inline=True)
        log_embed.add_field(name="Moderator", value=interaction.user.mention, inline=True)
        await send_unified_log(interaction.guild, "moderation", log_embed)
    except discord.Forbidden:
...
async def teamwarn(interaction: discord.Interaction, member: discord.Member, grund: str = "Kein Grund"):
    ...
    log_embed = discord.Embed(title="⚠️ User verwarnt", color=discord.Color.from_rgb(255, 200, 60), timestamp=datetime.now(timezone.utc))
    ...
    log_embed.add_field(name="Verwarnung Nr.", value=str(count), inline=True)
    log_embed.add_field(name="Grund", value=grund, inline=False)
    await send_unified_log(interaction.guild, "moderation", log_embed)
...
async def on_interaction(interaction: discord.Interaction):
    ...
    if cid.startswith("vs_accept:") or cid.startswith("vs_decline:"):
        ...
        try:
            ...
            if e.code != 40060:  # Interaction already acknowledged - ignorieren
                raise
...
async def on_voice_state_update(member: discord.Member, before: discord.VoiceState, after: discord.VoiceState):
    ...
    # Unified Log: Voice-Aktivität
    if before.channel != after.channel:
        if after.channel and not before.channel:
            log_embed = discord.Embed(title="🎙️ Voice beigetreten", color=discord.Color.from_rgb(100, 220, 150), timestamp=datetime.now(timezone.utc))
            log_embed.add_field(name="User", value=member.mention, inline=True)
            log_embed.add_field(name="Kanal", value=after.channel.mention, inline=True)
            await send_unified_log(guild, "voice", log_embed)
        elif before.channel and not after.channel:
            log_embed = discord.Embed(title="🔇 Voice verlassen", color=discord.Color.from_rgb(220, 80, 80), timestamp=datetime.now(timezone.utc))
            log_embed.add_field(name="User", value=member.mention, inline=True)
            log_embed.add_field(name="Kanal", value=before.channel.mention, inline=True)
            await send_unified_log(guild, "voice", log_embed)
        elif before.channel and after.channel:
            log_embed = discord.Embed(title="🔀 Voice gewechselt", color=discord.Color.from_rgb(255, 165, 0), timestamp=datetime.now(timezone.utc))
            log_embed.add_field(name="User", value=member.mention, inline=True)
...
async def on_message_delete(message: discord.Message):
    ...
    await send_unified_log(message.guild, "messages", embed)
...
async def on_message_edit(before: discord.Message, after: discord.Message):
    ...
    await send_unified_log(before.guild, "messages", embed)
...
async def on_member_join(member: discord.Member):
    ...
    # Unified Log
    log_embed = discord.Embed(title="✅ Mitglied beigetreten", color=discord.Color.from_rgb(100, 220, 150), timestamp=datetime.now(timezone.utc))
...
async def on_member_update(before: discord.Member, after: discord.Member):
    """Aktualisiert Teamliste wenn sich Rollen ändern + Ranklog + Logs"""
    # Nickname-Änderung loggen (Mitglieder-Kategorie)
    if before.nick != after.nick:
        log_embed = discord.Embed(title="✏️ Nickname geändert", color=discord.Color.from_rgb(255, 165, 0), timestamp=datetime.now(timezone.utc))
        log_embed.add_field(name="User", value=after.mention, inline=True)
        log_embed.add_field(name="Vorher", value=before.nick or before.name, inline=True)
        log_embed.add_field(name="Nachher", value=after.nick or after.name, inline=True)
        await send_unified_log(after.guild, "members", log_embed)

    if before.roles == after.roles:
        return

    # Unified Log: Rollenänderung (Server-Kategorie)
    added_roles_log = [r for r in after.roles if r not in before.roles]
    removed_roles_log = [r for r in before.roles if r not in after.roles]
    if added_roles_log or removed_roles_log:
        log_embed = discord.Embed(title="🎭 Rollen geändert", color=discord.Color.from_rgb(140, 180, 255), timestamp=datetime.now(timezone.utc))
        log_embed.add_field(name="User", value=after.mention, inline=False)
        if added_roles_log:
            log_embed.add_field(name="➕ Hinzugefügt", value=", ".join(r.mention for r in added_roles_log), inline=False)
        if removed_roles_log:
            log_embed.add_field(name="➖ Entfernt", value=", ".join(r.mention for r in removed_roles_log), inline=False)
        await send_unified_log(after.guild, "server", log_embed)
...
# ─────────────────────────────────────────────
# SCAM / KRYPTO-WERBUNG ERKENNUNG
# ─────────────────────────────────────────────
# Erkennt Scam-Werbung (z.B. gefälschte Promi-Accounts mit Krypto-Bonus-Versprechen)
# durch Kombination mehrerer verdächtiger Signale, statt einzelner Wörter,
# um normale Gespräche über Krypto nicht fälschlich zu blockieren.

SCAM_MONEY_TERMS = [
    "usdt", "bitcoin", "btc", "eth", "ethereum", "crypto", "kryptowährung",
    "wallet", "airdrop", "binance", "coinbase",
]
SCAM_BAIT_TERMS = [
    "bonus", "giveaway", "verlosung", "geschenkt", "gratis geld", "free money",
    "withdraw", "auszahlung", "gewinn sofort", "claim your reward", "claim now",
]
SCAM_URGENCY_TERMS = [
    "limited time", "begrenzte zeit", "first", "erste", "schnell sein",
    "nur für kurze zeit", "wird gelöscht", "will be deleted", "verpass nicht",
    "don't miss",
]
SCAM_CODE_TERMS = [
    "promo code", "promo-code", "rabattcode", "gutscheincode", "enter the code",
    "code eingeben", "referral", "invite code",
]

def detect_scam_advertising(text: str) -> bool:
    """Gibt True zurück wenn der Text wahrscheinlich Krypto/Scam-Werbung ist.
    Erfordert eine Kombination aus mehreren Signalkategorien (Geld + Köder/Code/Dringlichkeit),
    damit normale Nachrichten (z.B. 'hab mir Bitcoin gekauft') nicht fälschlich erfasst werden."""
    normalized = normalize_text(text)
    if len(text) < 15:
        return False

    has_money = any(term.replace(" ", "") in normalized for term in SCAM_MONEY_TERMS)
    has_bait = any(term.replace(" ", "") in normalized for term in SCAM_BAIT_TERMS)
    has_urgency = any(term.replace(" ", "") in normalized for term in SCAM_URGENCY_TERMS)
    has_code = any(term.replace(" ", "") in normalized for term in SCAM_CODE_TERMS)

    signal_count = sum([has_money, has_bait, has_urgency, has_code])
    # Mindestens 2 unterschiedliche Signalkategorien müssen zutreffen
    if has_money and (has_bait or has_code):
        return True
    if signal_count >= 3:
        return True
    return False
...
async def on_message_automod(message: discord.Message):
    ...
    is_scam = False
    if cfg.get("scam_detection", True):  # standardmäßig aktiv
        is_scam = detect_scam_advertising(message.content)

    if not found_word and not is_scam:
        return
    ...
    if log_channel_id:
        ...
        if log_ch:
            if is_scam and not found_word:
                embed = discord.Embed(
                    title="🚨 Automod – Scam/Werbung erkannt & gelöscht",
            ...
            if is_scam:
                embed.add_field(name="⚠️ Grund", value="Verdächtige Krypto/Scam-Werbung", inline=True)
...
@app_commands.describe(
    ...
    scam_erkennung="Krypto/Scam-Werbung automatisch erkennen und löschen (Standard: An)"
)
@app_commands.choices(aktion=[
...
async def automod_setup(
    ...
    cfg["scam_detection"] = scam_erkennung
    if log_kanal:
    ...
    scam_status = "✅ aktiviert" if scam_erkennung else "❌ deaktiviert"
    ...
    await interaction.followup.send(
        embed=liquid_glass_embed(
            ...
            f"**Scam/Krypto-Werbung erkennen:** {scam_status}\n"
