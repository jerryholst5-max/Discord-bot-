...
async def on_voice_state_update(member: discord.Member, before: discord.VoiceState, after: discord.VoiceState):
    ...
    # Prüfen ob User einen laufenden Support verlassen hat (vs_close Button vorhanden)
    if before.channel and (not after.channel or after.channel != before.channel):
        for system_num, vs_cfg in [
            ...
            if not notif_channel_id or not support_category_id:
                continue
            # Prüfen ob before.channel in der Support-Kategorie war
            ...
            try:
                async for msg in notif_channel.history(limit=100):
                    ...
                    for row in msg.components:
                        for btn in row.children:
                            if hasattr(btn, "custom_id") and btn.custom_id and btn.custom_id.startswith(f"vs_close:{member.id}:"):
                                found = True
                                # Dauer berechnen aus start_ts im custom_id
                                parts = btn.custom_id.split(":")
                                dauer_text = ""
                                if len(parts) >= 6:
                                    try:
                                        import time as _time
                                        start_ts = int(parts[5])
                                        dauer_sek = int(_time.time()) - start_ts
                                        minuten = dauer_sek // 60
                                        sekunden = dauer_sek % 60
                                        dauer_text = f"\n⏱️ **Dauer:** {minuten}m {sekunden}s"
                                    except Exception:
                                        pass
