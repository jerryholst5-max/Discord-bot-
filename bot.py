...
async def on_member_remove(member):
    ...
    if bye_ch_id:
        ...
        if bye_ch:
            ...
            msg = msg.replace("{user}", str(member)).replace("{server}", guild.name).replace("{count}", str(guild.member_count))
            await bye_ch.send(msg)
...
async def on_member_join(member: discord.Member):
    ...
    if wc_ch_id:
        ...
        if wc_ch:
            ...
            msg = msg.replace("{user}", member.mention).replace("{server}", guild.name).replace("{count}", str(guild.member_count))
            await wc_ch.send(msg)
    # Auto-Rolle
