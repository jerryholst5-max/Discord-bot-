...
async def partner_hinzufuegen(interaction: discord.Interaction, name: str, link: str, kategorie: str = "🤝 Partner", ansprechpartner: str = None, beschreibung: str = None):
    ...
    if any(p["name"].lower() == name.lower() for p in partners):
        ...
        return
    from datetime import datetime as _dt
    partners.append({"name": name, "link": link, "kategorie": kategorie, "ansprechpartner": ansprechpartner or "", "beschreibung": beschreibung or "", "datum": _dt.now().strftime("%d.%m.%Y"), "hinzugefuegt_von": str(interaction.user), "sichtbar": True})
...
async def partner_liste(interaction: discord.Interaction):
    ...
    if not partners:
        ...
        return
    lines = [f"`{i}.` **[{p['name']}]({'https://' + p['link'] if not p['link'].startswith('http') else p['link']})** • {p.get('kategorie','🤝')} • {'🌐' if p.get('sichtbar', True) else '🚫'}" for i, p in enumerate(partners, 1)]
