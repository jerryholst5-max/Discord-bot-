...
async def ticket_setup(
    ...
    panel_msg = await kanal.send(embed=embed, view=TicketView(custom_for_view))
...
async def ticket_panel_aktualisieren(interaction: discord.Interaction, kanal: discord.TextChannel):
    ...
    if panel_msg_id and panel_channel_id:
        try:
            panel_channel = interaction.guild.get_channel(int(panel_channel_id))
            if panel_channel:
                old_msg = await panel_channel.fetch_message(int(panel_msg_id))
                await old_msg.edit(embed=embed, view=TicketView(custom_for_view))
                edited = True
        except Exception:
            pass

    if not edited:
        # Post new panel and save message ID
        msg = await kanal.send(embed=embed, view=TicketView(custom_for_view))
    ...
    await interaction.followup.send(
        embed=liquid_glass_embed(
            "✅ Panel aktualisiert!",
            f"Das Ticket-Panel wurde mit allen Kategorien aktualisiert:\n{cat_names}",
            discord.Color.from_rgb(100, 220, 150)
        ),
        ephemeral=True
    )
