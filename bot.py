...
async def on_ready():
    ...
    try:
        ...
        bot.add_view(TicketNotifView(accepted=True))
    ...
    try:
        ...
        # Guild-spezifischer Sync für sofortige Aktualisierung in allen Guilds
...
@tree.command(name="level-xp-nachricht", description="XP pro Nachricht einstellen (Admin)")
@app_commands.describe(xp="XP die pro Nachricht vergeben werden (z.B. 20)")
async def level_xp_nachricht(interaction: discord.Interaction, xp: int):
    if not interaction.user.guild_permissions.administrator:
        await interaction.response.send_message("❌ Keine Berechtigung!", ephemeral=True)
        return
    await interaction.response.defer(ephemeral=True)
    cfg = await get_level_config(interaction.guild.id)
    cfg["xp_pro_nachricht"] = xp
    await save_level_config(interaction.guild.id, cfg)
    await interaction.followup.send(
        embed=liquid_glass_embed("✅ XP pro Nachricht gesetzt", f"Ab jetzt gibt es **{xp} XP** pro Nachricht.", discord.Color.from_rgb(255, 215, 0)),
        ephemeral=True
    )

@tree.command(name="level-xp-voice", description="XP pro Voice-Minute einstellen (Admin)")
@app_commands.describe(xp="XP die pro Minute im Voice-Channel vergeben werden (z.B. 5)")
async def level_xp_voice(interaction: discord.Interaction, xp: int):
    if not interaction.user.guild_permissions.administrator:
        await interaction.response.send_message("❌ Keine Berechtigung!", ephemeral=True)
        return
    await interaction.response.defer(ephemeral=True)
    ...
    await save_level_config(interaction.guild.id, cfg)
    await interaction.followup.send(
        embed=liquid_glass_embed("✅ XP pro Voice-Minute gesetzt", f"Ab jetzt gibt es **{xp} XP** pro Minute im Voice-Channel.", discord.Color.from_rgb(255, 215, 0)),
        ephemeral=True
    )
