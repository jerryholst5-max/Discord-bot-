...
class TicketView(discord.ui.View):
    def __init__(self, custom_categories: dict = None):
        super().__init__(timeout=None)
        cats = dict(TICKET_CATEGORIES)
        if custom_categories:
            cats.update(custom_categories)
...
async def ticket_setup(
    ...
    custom_for_view = {k: (e, l, d) for k, (e, l, d) in all_cats.items() if k not in TICKET_CATEGORIES}
    await kanal.send(embed=embed, view=TicketView(custom_for_view))
