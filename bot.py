...
class VoiceSupportView(discord.ui.View):
    ...
    async def _accept_callback(self, interaction: discord.Interaction):
        ...
        # Konfigurierte Embed-Rollen laden
        if self.system_num == 2:
            embed_cfg = await get_voice_support_2_config(guild.id)
        else:
            embed_cfg = await get_voice_support_config(guild.id)
        embed_user_role_id = embed_cfg.get("embed_user_role_id") if embed_cfg else None
        embed_support_role_id = embed_cfg.get("embed_support_role_id") if embed_cfg else None
        user_role = guild.get_role(int(embed_user_role_id)) if embed_user_role_id else None
        support_role_display = guild.get_role(int(embed_support_role_id)) if embed_support_role_id else None
        user_display = user_role.mention if user_role else member.mention
        support_display = support_role_display.mention if support_role_display else support_member.mention
    ...
    async def _decline_callback(self, interaction: discord.Interaction):
        ...
        # User aus dem Warteraum schmeißen - nur wenn in der konfigurierten Kategorie
        kicked_from_voice = False
        if member and member.voice and member.voice.channel:
            try:
                # Konfigurierte Kategorie für dieses System laden
                if self.system_num == 2:
                    cfg = await load_voice_support_config_2(guild.id)
                else:
                    cfg = await load_voice_support_config(guild.id)

                category_id = int(cfg.get("support_category_id", 0)) if cfg else 0
                warteraum_id = int(cfg.get("warteraum_id", 0)) if cfg else 0

                user_channel = member.voice.channel
                in_category = (
                    user_channel.category_id == category_id or
                    user_channel.id == warteraum_id
                )
    ...
    for system_num, vs_cfg in [
        ...
        if warteraum_id and notif_channel_id:
            if after.channel and str(after.channel.id) == str(warteraum_id):
                ...
                if notif_channel:
                    # Abwesenheitsrolle prüfen: Mitglieder mit der Rolle werden beim Ping gefiltert
                    cfg = await load_config()
                    ...
                    if ping_role_id:
                        if abwesenheits_role_id:
                            ...
                            if abwesenheits_role and ping_role:
                                # Nur Mitglieder der Ping-Rolle ohne Abwesenheitsrolle erwähnen
                                active_members = [
...
class AbmeldungModal(discord.ui.Modal, title="Abmeldung"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # Abwesenheitsrolle vergeben falls konfiguriert
...
class AbmeldungView(discord.ui.View):
    ...
    async def zurueck_melden(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if role not in interaction.user.roles:
            await interaction.response.send_message("ℹ️ Du hast die Abwesenheitsrolle gar nicht.", ephemeral=True)
            return
        try:
...
async def on_member_update(before: discord.Member, after: discord.Member):
    """Aktualisiert Teamliste wenn sich Rollen ändern"""
    if before.roles != after.roles:
        cfg = await get_teamliste_config(after.guild.id)
        if not cfg:
            return
        role_ids = [r.get("role_id") for r in cfg.get("roles", [])]
        # Check if changed role is in teamliste
