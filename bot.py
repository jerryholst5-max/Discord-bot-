...
async def update_teamliste(guild: discord.Guild):
    ...
    for role_entry in roles:
        ...
        member_str = "\n".join(f"@{m.display_name}" for m in members) if members else "*Niemand*"
        field_name = f"@ | {label or role.name} ({len(members)})"
        # Split if too long
        if len(member_str) > 1024:
            member_str = member_str[:1020] + "..."
        embed.add_field(name=field_name, value=member_str, inline=False)
    
    embed.set_footer(text=f"Last Updated at {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M:%S')}")
