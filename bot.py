...
# ─────────────────────────────────────────────
# KI-Scam-Detektor (über OpenRouter)
# ─────────────────────────────────────────────

OPENROUTER_KEY = os.environ.get("OPENROUTER_KEY", "")
OPENROUTER_MODEL = os.environ.get("OPENROUTER_MODEL", "anthropic/claude-haiku-4.5")

async def ai_scam_check_text(text: str) -> tuple[bool, str]:
    """
    Prüft einen Text auf Scam. Gibt (is_scam, reason) zurück.
    """
    if not OPENROUTER_KEY or not text.strip():
        return False, ""
    prompt = (
        "Du bist ein Scam-Erkennungs-Assistent für einen Discord-Server. "
        "Analysiere die folgende Nachricht und entscheide ob es sich um Scam, Betrug, Phishing oder eine gefährliche Werbung handelt. "
        "Typische Scam-Muster: Krypto-Giveaways, 'Klicke auf diesen Link und erhalte Geld', gefälschte Gewinne, Promo-Codes für Casino/Crypto, "
        "Aufforderungen persönliche Daten einzugeben, verdächtige URLs, Nachrichten die Dringlichkeit erzeugen ('wird in 1 Stunde gelöscht'). "
        "Antworte NUR mit JSON: {\"scam\": true/false, \"grund\": \"kurze Erklärung auf Deutsch (max 100 Zeichen)\"}\n\n"
        f"Nachricht: {text[:1500]}"
    )
    try:
        async with aiohttp.ClientSession() as session:
            async with session.post(
                ...
                return bool(result.get("scam")), result.get("grund", "")
    except Exception as e:
        print(f"[SCAM-TEXT] Fehler: {e}")
        return False, ""


async def ai_scam_check_image(image_url: str) -> tuple[bool, str]:
    """
    Lädt ein Bild herunter und prüft es auf Scam-Inhalte. Gibt (is_scam, reason) zurück.
    """
    ...
    try:
        async with aiohttp.ClientSession() as session:
            # Bild herunterladen
            async with session.get(image_url, timeout=aiohttp.ClientTimeout(total=10)) as img_resp:
                ...
                if len(img_data) > 5 * 1024 * 1024:  # max 5MB
                    return False, ""
                img_b64 = base64.standard_b64encode(img_data).decode()

            # Vision-Modell über OpenRouter anfragen
            async with session.post(
                "https://openrouter.ai/api/v1/chat/completions",
                headers={
                    "Authorization": f"Bearer {OPENROUTER_KEY}",
                    ...
                    "messages": [{
                        ...
                        "content": [
                            ...
                            },
                            {
                                "type": "text",
                                "text": (
                                    "Du bist ein Scam-Erkennungs-Assistent. "
                                    "Analysiere dieses Bild: Enthält es Scam, Betrug, Phishing, gefälschte Gewinne, "
                                    "Krypto-Giveaways, verdächtige Links, Casino-Werbung oder andere betrügerische Inhalte? "
                                    "Antworte NUR mit JSON: {\"scam\": true/false, \"grund\": \"kurze Erklärung auf Deutsch (max 100 Zeichen)\"}"
                                ),
                            },
                        ],
...
async def on_message_automod(message: discord.Message):
    ...
    scam_detected = False
    scam_reason = ""
    scam_source = ""  # "text" oder "bild"

    if cfg.get("scam_check_enabled", False):
        # Text prüfen (nur wenn kein bad_word schon gefunden)
        if not found_word and message.content.strip():
            scam_detected, scam_reason = await ai_scam_check_text(message.content)
            if scam_detected:
                scam_source = "text"
        ...
        if not scam_detected:
            for attachment in message.attachments:
                if any(attachment.filename.lower().endswith(ext) for ext in (".png", ".jpg", ".jpeg", ".gif", ".webp")):
                    scam_detected, scam_reason = await ai_scam_check_image(attachment.url)
                    if scam_detected:
                        scam_source = "bild"
                        break

    # ── Abbrechen wenn nichts gefunden ─────────────────────────────────────
    if not found_word and not scam_detected:
        return
    ...
    if log_channel_id:
        ...
        if log_ch:
            if scam_detected:
                ...
                embed.add_field(name="📌 Kanal", value=message.channel.mention, inline=True)
                embed.add_field(name="🔍 Erkannt in", value=f"`{scam_source}`", inline=True)
                embed.add_field(name="⚠️ Grund", value=scam_reason or "KI-Scam-Erkennung", inline=False)
                if message.content:
                    embed.add_field(name="💬 Nachricht", value=message.content[:500], inline=False)
                if message.attachments:
                    embed.add_field(name="🖼️ Anhänge", value="\n".join(a.filename for a in message.attachments), inline=False)
                embed.set_footer(text=f"ID: {message.author.id} • KI-Scam-Filter")
            else:
                embed = discord.Embed(
    ...
    # Aktion ausführen
    warn_text = (
        f"{message.author.mention} ⚠️ Deine Nachricht wurde als **Scam/Betrug** erkannt und gelöscht."
    ...
    if action == "mute":
        ...
        try:
            ...
            reason = f"Automod Scam: {scam_reason}" if scam_detected else f"Automod: {found_word}"
...
@app_commands.describe(
    ...
    ki_scam_check="KI-Scam-Erkennung für Texte und Bilder aktivieren"
)
@app_commands.choices(aktion=[
...
async def automod_setup(
    ...
    cfg["scam_check_enabled"] = ki_scam_check
    if log_kanal:
    ...
    scam_status = "✅ aktiviert" if ki_scam_check else "❌ deaktiviert"
    ...
    await interaction.followup.send(
        embed=liquid_glass_embed(
            ...
            f"**🤖 KI-Scam-Erkennung:** {scam_status}\n\n"
