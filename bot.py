...
class VoiceSupportCloseView(discord.ui.View):
    ...
    async def _close_callback(self, interaction: discord.Interaction):
        ...
        if member and member.voice and member.voice.channel:
            try:
                # Nur rauswerfen wenn Member noch in der Support-Kategorie ist
                # (gleiche Kategorie wie der ursprüngliche Support-Channel)
                support_channel = guild.get_channel(self.target_channel_id)
                support_category = support_channel.category if support_channel else None
                member_category = member.voice.channel.category

                if support_category and member_category and member_category.id == support_category.id:
                    await member.move_to(None)
                elif support_channel and member.voice.channel.id == support_channel.id:
                    await member.move_to(None)
            except Exception as e:
