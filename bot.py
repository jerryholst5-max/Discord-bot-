...
async def update_teamliste(guild: discord.Guild):
    ...
    for role_entry in roles:
        ...
        # Echte Discord-Mentions für jeden Member
        member_str = "\n".join(f"{m.mention}" for m in members) if members else "*Niemand*"

        # Rollenfarbe als Hex-String (z.B. #FFD700) für den Feldnamen
        # Discord Embed field names unterstützen kein Markdown-Farbe,
        # daher nutzen wir die Rollenfarbe als Embed-Akzentfarbe UND
        # zeigen den Rollennamen als farbige Rolle-Mention im value an.
        role_color = role.color
        display_label = label or role.name

        # Field name: "@ | 👑 • RolleName (Anzahl)"
        field_name = f"@ | {display_label} ({len(members)})"

        # Rollenname als farbige Mention + Mitglieder darunter
        # Die Rollenfarbe erscheint durch die Rolle-Mention automatisch in Discord
