...
class VerifyButtonView(discord.ui.View):
    ...
    async def verifizieren(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if not rolle_id or not anfrage_kanal_id or not pruefer_rolle_id:
        ...
        if not rolle or not anfrage_kanal:
        ...
        await interaction.response.send_message("📨 Deine Verifizierungs-Anfrage wurde gesendet. Bitte warte, bis das Team sie bestätigt.", ephemeral=True)

        embed = liquid_glass_embed(
            "📨 Neue Verifizierungs-Anfrage",
            f"{interaction.user.mention} (`{interaction.user.id}`) möchte sich verifizieren.",
            discord.Color.from_rgb(240, 165, 0)
        )
        embed.set_thumbnail(url=interaction.user.display_avatar.url)
...
class VerifyAnfrageView(discord.ui.View):
    ...
    async def annehmen(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        rolle = interaction.guild.get_role(int(cfg.get("verified_role_id")))
        antragsteller = interaction.guild.get_member(int(self.antragsteller_id))
        if not rolle or not antragsteller:
            await interaction.response.send_message("❌ Nutzer oder Rolle nicht mehr gefunden.", ephemeral=True)
            return
        try:
            await antragsteller.add_roles(rolle, reason=f"Verifizierung bestätigt von {interaction.user}")
        except discord.Forbidden:
            await interaction.response.send_message("❌ Mir fehlt die Berechtigung, die Rolle zu vergeben.", ephemeral=True)
            return
...
async def verify_setup(interaction: discord.Interaction, verify_kanal: discord.TextChannel, verifiziert_rolle: discord.Role, anfrage_kanal: discord.TextChannel, pruefer_rolle: discord.Role, log_kanal: discord.TextChannel = None):
    if not interaction.user.guild_permissions.administrator:
        await interaction.response.send_message("❌ Keine Berechtigung!", ephemeral=True)
        return
