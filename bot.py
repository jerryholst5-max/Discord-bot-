...
def load_config_sync() -> dict:
    """Synchrone Version von load_config für Event-Handler"""
    global _config_cache
    if _config_cache is not None:
        return _config_cache
    return {"alert_users": []}
...
def save_config_sync(data: dict):
    """Synchrone Version von save_config (nur Cache)"""
    global _config_cache
    _config_cache = data
...
_nuke_tracker = defaultdict(lambda: defaultdict(list))  # guild_id -> user_id -> [timestamps]
NUKE_WINDOW = 10   # seconds
NUKE_THRESHOLD = 3  # actions before triggering

def get_antinuke_config(guild_id: int) -> dict:
    cfg = load_config_sync()
    return cfg.get("antinuke", {}).get(str(guild_id), {
        "enabled": False,
        "immune_roles": [],
        "immune_users": [],
    })

def save_antinuke_config(guild_id: int, data: dict):
    cfg = load_config_sync()
    if "antinuke" not in cfg:
        cfg["antinuke"] = {}
    cfg["antinuke"][str(guild_id)] = data
    save_config_sync(cfg)

def is_nuke_immune(member: discord.Member, cfg: dict) -> bool:
    if member.id == OWNER_ID:
        return True
    if member.id in cfg.get("immune_users", []):
        return True
    member_role_ids = [r.id for r in member.roles]
    if any(r in member_role_ids for r in cfg.get("immune_roles", [])):
        return True
    return False

async def handle_nuke_action(guild: discord.Guild, member: discord.Member, action: str):
    cfg = get_antinuke_config(guild.id)
    if not cfg.get("enabled"):
        return
    if is_nuke_immune(member, cfg):
        return
    ...
    _nuke_tracker[guild_id][user_id] = [
        t for t in _nuke_tracker[guild_id][user_id]
        if now - t < NUKE_WINDOW
    ]
    _nuke_tracker[guild_id][user_id].append(now)

    if len(_nuke_tracker[guild_id][user_id]) >= NUKE_THRESHOLD:
        _nuke_tracker[guild_id][user_id].clear()
        try:
            await member.ban(reason=f"Anti-Nuke: Zu viele {action} Aktionen")
        except Exception:
            pass
        # Log to first available text channel
        for ch in guild.text_channels:
            if ch.permissions_for(guild.me).send_messages:
                embed = liquid_glass_embed(
                    "🛡️ Anti-Nuke ausgelöst!",
                    f"**{member}** wurde gebannt!\n**Grund:** Zu viele `{action}` Aktionen in {NUKE_WINDOW} Sekunden.",
                    discord.Color.from_rgb(255, 50, 50)
                )
                await ch.send(embed=embed)
                break
...
async def on_guild_channel_delete(channel):
    ...
    async for entry in guild.audit_logs(limit=1, action=discord.AuditLogAction.channel_delete):
        if entry.user and not entry.user.bot:
            await handle_nuke_action(guild, entry.user, "Kanal löschen")
...
async def on_guild_role_delete(role):
    ...
    async for entry in guild.audit_logs(limit=1, action=discord.AuditLogAction.role_delete):
        if entry.user and not entry.user.bot:
            await handle_nuke_action(guild, entry.user, "Rolle löschen")

@bot.event
async def on_member_ban(guild, user):
    async for entry in guild.audit_logs(limit=1, action=discord.AuditLogAction.ban):
        if entry.user and not entry.user.bot:
            await handle_nuke_action(guild, entry.user, "Ban")
...
async def on_webhooks_update(channel):
    ...
    async for entry in guild.audit_logs(limit=1, action=discord.AuditLogAction.webhook_create):
        if entry.user and not entry.user.bot:
            await handle_nuke_action(guild, entry.user, "Webhook erstellen")

# /antinuke
@tree.command(name="antinuke", description="Anti-Nuke aktivieren oder deaktivieren")
@app_commands.describe(aktiv="Anti-Nuke ein- oder ausschalten")
async def antinuke(interaction: discord.Interaction, aktiv: bool):
    if not is_owner(interaction.user):
        await interaction.response.send_message("Nur der Owner kann Anti-Nuke einstellen!", ephemeral=True)
        return
...
# /antinuke-immune-rolle
@tree.command(name="antinuke-immune-rolle", description="Rolle vom Anti-Nuke immunisieren/entfernen")
@app_commands.describe(rolle="Die Rolle", aktion="Hinzufügen oder entfernen")
@app_commands.choices(aktion=[
    app_commands.Choice(name="hinzufügen", value="add"),
    app_commands.Choice(name="entfernen", value="remove"),
])
async def antinuke_immune_rolle(interaction: discord.Interaction, rolle: discord.Role, aktion: str):
    if not is_owner(interaction.user):
        await interaction.response.send_message("Nur der Owner!", ephemeral=True)
        return
    cfg = get_antinuke_config(interaction.guild.id)
    immune = cfg.get("immune_roles", [])
    if aktion == "add":
        if rolle.id not in immune:
            immune.append(rolle.id)
        msg = f"**{rolle.name}** ist jetzt immun."
    else:
        immune = [r for r in immune if r != rolle.id]
        msg = f"**{rolle.name}** ist nicht mehr immun."
    cfg["immune_roles"] = immune
    save_antinuke_config(interaction.guild.id, cfg)
    await interaction.response.send_message(embed=liquid_glass_embed("🛡️ Anti-Nuke • Rolle", msg, discord.Color.from_rgb(130, 200, 240)))

# /antinuke-immune-user
@tree.command(name="antinuke-immune-user", description="Bis zu 5 User vom Anti-Nuke immunisieren/entfernen")
@app_commands.describe(
    aktion="Hinzufügen oder entfernen",
    user1="User 1", user2="User 2 (optional)", user3="User 3 (optional)",
    user4="User 4 (optional)", user5="User 5 (optional)"
)
@app_commands.choices(aktion=[
    app_commands.Choice(name="hinzufügen", value="add"),
    app_commands.Choice(name="entfernen", value="remove"),
])
async def antinuke_immune_user(
    interaction: discord.Interaction,
    aktion: str,
    ...
    if not is_owner(interaction.user):
        ...
        return
    cfg = get_antinuke_config(interaction.guild.id)
    immune = cfg.get("immune_users", [])
    users = [u for u in [user1, user2, user3, user4, user5] if u is not None]
    names = []
    for u in users:
        if aktion == "add":
            if u.id not in immune:
                immune.append(u.id)
            names.append(f"✅ {u.mention}")
        else:
            immune = [i for i in immune if i != u.id]
            names.append(f"❌ {u.mention}")
    cfg["immune_users"] = immune
    save_antinuke_config(interaction.guild.id, cfg)
    action_text = "immunisiert" if aktion == "add" else "entfernt"
    await interaction.response.send_message(
        embed=liquid_glass_embed(
            f"🛡️ Anti-Nuke • User {action_text}",
            "\n".join(names),
            discord.Color.from_rgb(130, 200, 240)
        )
    )
...
# /antinuke-status
@tree.command(name="antinuke-status", description="Zeigt die Anti-Nuke Einstellungen")
async def antinuke_status(interaction: discord.Interaction):
    ...
    immune_users = cfg.get("immune_users", [])
    immune_roles = cfg.get("immune_roles", [])
    users_str = ", ".join(f"<@{u}>" for u in immune_users) if immune_users else "Keine"
    roles_str = ", ".join(f"<@&{r}>" for r in immune_roles) if immune_roles else "Keine"
    embed = liquid_glass_embed(
        "🛡️ Anti-Nuke Status",
        f"**Status:** {'✅ Aktiv' if enabled else '❌ Inaktiv'}\n**Immune User:** {users_str}\n**Immune Rollen:** {roles_str}\n\n**Schwellenwert:** {NUKE_THRESHOLD} Aktionen in {NUKE_WINDOW} Sekunden",
        discord.Color.from_rgb(130, 200, 240)
    )
    await interaction.response.send_message(embed=embed, ephemeral=True)
