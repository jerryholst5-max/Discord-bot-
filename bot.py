...
async def help_cmd(interaction: discord.Interaction):
    ...
    embed.add_field(
        name="📊 Team Dashboard",
        value="`/team_dashboard` – Team-Statistiken anzeigen\n`/team_stats_reset` – Stats zurücksetzen (Owner)",
        inline=False
    )
...
async def on_member_join(member: discord.Member):
    ...
    # Join Log
    channel_id = log_cfg.get("join_leave_log")
    if channel_id:
        channel = guild.get_channel(int(channel_id))
        if channel:
    ...
    wc_channel_id = welcome_cfg.get("channel_id")
    wc_message = welcome_cfg.get("message", f"Willkommen {member.mention} auf **{guild.name}**! 🎉")
    if wc_channel_id:
        wc_channel = guild.get_channel(int(wc_channel_id))
        if wc_channel:
            msg = wc_message.replace("{user}", member.mention).replace("{server}", guild.name).replace("{count}", str(guild.member_count))
            await wc_channel.send(msg)

    # Auto-Rolle
    auto_role_id = cfg.get("auto_role", {}).get(str(guild.id))
    if auto_role_id:
...
async def on_member_remove(member: discord.Member):
    ...
    bye_channel_id = bye_cfg.get("channel_id")
    bye_message = bye_cfg.get("message", f"**{member}** hat den Server verlassen. 👋")
    if bye_channel_id:
        bye_channel = guild.get_channel(int(bye_channel_id))
        if bye_channel:
            msg = bye_message.replace("{user}", str(member)).replace("{server}", guild.name)
            await bye_channel.send(msg)

# Ban Logs
@bot.event
async def on_member_ban(guild: discord.Guild, user: discord.User):
    embed = discord.Embed(title="🔨 User gebannt", color=discord.Color.from_rgb(220, 50, 50), timestamp=datetime.now(timezone.utc))
    embed.add_field(name="User", value=f"{user.mention} ({user})", inline=False)
    await send_log(guild, "ban_log", embed)
...
@app_commands.describe(
    ...
    nachricht="Nachricht ({user} = Mention, {server} = Servername, {count} = Memberanzahl)"
)
async def willkommen_setup(interaction: discord.Interaction, kanal: discord.TextChannel, nachricht: str = None):
    if not has_permission(interaction, "administrator"):
        await interaction.response.send_message("Keine Berechtigung!", ephemeral=True)
        return
    await interaction.response.defer(ephemeral=True)
    ...
    await save_config(cfg)
    await interaction.followup.send(embed=liquid_glass_embed("✅ Willkommen-Setup", f"Kanal: {kanal.mention}\nNachricht gesetzt.", discord.Color.from_rgb(100, 220, 150)), ephemeral=True)
...
@app_commands.describe(
    ...
    nachricht="Nachricht ({user} = Username, {server} = Servername)"
)
async def abschied_setup(interaction: discord.Interaction, kanal: discord.TextChannel, nachricht: str = None):
    if not has_permission(interaction, "administrator"):
        await interaction.response.send_message("Keine Berechtigung!", ephemeral=True)
        return
    await interaction.response.defer(ephemeral=True)
    ...
    cfg["goodbye"][str(interaction.guild.id)] = {
        "channel_id": str(kanal.id),
        "message": nachricht or "**{user}** hat **{server}** verlassen. 👋"
    }
    await save_config(cfg)
    await interaction.followup.send(embed=liquid_glass_embed("✅ Abschied-Setup", f"Kanal: {kanal.mention}\nNachricht gesetzt.", discord.Color.from_rgb(100, 220, 150)), ephemeral=True)

@tree.command(name="auto-rolle", description="Setzt die Auto-Rolle bei Beitritt (Admin)")
@app_commands.describe(rolle="Rolle die neue Mitglieder automatisch bekommen")
async def auto_rolle(interaction: discord.Interaction, rolle: discord.Role):
    if not has_permission(interaction, "administrator"):
        await interaction.response.send_message("Keine Berechtigung!", ephemeral=True)
        return
    await interaction.response.defer(ephemeral=True)
    ...
    if "auto_role" not in cfg:
        cfg["auto_role"] = {}
    cfg["auto_role"][str(interaction.guild.id)] = str(rolle.id)
    await save_config(cfg)
    await interaction.followup.send(embed=liquid_glass_embed("✅ Auto-Rolle", f"Neue Mitglieder bekommen automatisch {rolle.mention}.", discord.Color.from_rgb(100, 220, 150)), ephemeral=True)
...
async def modlog_setup(interaction: discord.Interaction, kanal: discord.TextChannel):
    ...
    await interaction.followup.send(
        embed=liquid_glass_embed(
            "✅ Modlog eingerichtet",
            f"Bans und Kicks werden ab jetzt in {kanal.mention} eingetragen.",
            discord.Color.from_rgb(100, 220, 150)
        ),
        ephemeral=True
    )
...
# ─────────────────────────────────────────────
# /abmeldung-setup – Abmeldungs-System
# ─────────────────────────────────────────────
