...
async def get_all_backups(guild_id: int) -> list:
    try:
        db = get_db()
        col = db["backups"]
        cursor = col.find({"guild_id": str(guild_id)}).sort("timestamp", -1)
        return await cursor.to_list(length=50)
    except Exception:
        return []

async def save_backup_to_db(guild_id: int, backup_data: dict):
    try:
        db = get_db()
        col = db["backups"]
        await col.insert_one(backup_data)
        all_backups = await col.find({"guild_id": str(guild_id)}).sort("timestamp", -1).to_list(length=50)
...
async def backup_erstellen(interaction: discord.Interaction):
    ...
    try:
        ...
        await save_backup_to_db(interaction.guild.id, backup_data)
        import datetime
...
async def backup_liste(interaction: discord.Interaction):
    if not interaction.user.guild_permissions.administrator:
        ...
        return
    await interaction.response.defer(ephemeral=True)
    backups = await get_all_backups(interaction.guild.id)
    if not backups:
    ...
    for i, b in enumerate(backups, 1):
        ...
        lines.append(f"**{i}.** {ts} — {auto_text} | {rollen} Rollen | {kategorien} Kategorien")
    await interaction.followup.send(
        embed=liquid_glass_embed(
            "💾 Backup-Liste",
            "\n".join(lines),
