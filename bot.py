...
def _generate_captcha_image() -> tuple[str, discord.File]:
    ...
    breite, hoehe = 620, 220
    bild = Image.new("RGB", (breite, hoehe), color=(25, 25, 30))
    draw = ImageDraw.Draw(bild)

    # Rausch-Linien im Hintergrund
    for _ in range(12):
        start = (random.randint(0, breite), random.randint(0, hoehe))
        ende = (random.randint(0, breite), random.randint(0, hoehe))
        draw.line([start, ende], fill=(random.randint(60, 100), random.randint(60, 100), random.randint(60, 100)), width=3)

    try:
        schrift = ImageFont.truetype("/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf", 110)
    except Exception:
        schrift = ImageFont.load_default()

    x = 30
    for zeichen_einzel in code:
        y = random.randint(25, 60)
        winkel = random.randint(-20, 20)
        zeichen_bild = Image.new("RGBA", (130, 150), (0, 0, 0, 0))
        zeichen_draw = ImageDraw.Draw(zeichen_bild)
        farbe = (random.randint(200, 255), random.randint(160, 210), random.randint(20, 60))
        zeichen_draw.text((10, 5), zeichen_einzel, font=schrift, fill=farbe)
        zeichen_bild = zeichen_bild.rotate(winkel, expand=1)
        bild.paste(zeichen_bild, (x, y), zeichen_bild)
        x += 92

    # Rausch-Linien über den Zeichen (damit sie das Captcha wirklich durchkreuzen)
    for _ in range(14):
        start = (random.randint(0, breite), random.randint(0, hoehe))
        ende = (random.randint(0, breite), random.randint(0, hoehe))
        draw.line([start, ende], fill=(random.randint(90, 150), random.randint(90, 150), random.randint(90, 150)), width=3)
