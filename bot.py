...
async def on_message(message: discord.Message):
    if message.author.bot or not message.guild:
        return
    cfg = await get_level_config(message.guild.id)
    if not cfg.get("enabled"):
        return
    # Cooldown prüfen
    ...
    if _xp_cooldowns.get(key, 0) > now:
        return
    _xp_cooldowns[key] = now + 60  # 1 Minute Cooldown
    import random
...
async def get_partner_config(guild_id: int) -> dict:
    cfg = await load_config()
    return cfg.get("partner_config", {}).get(str(guild_id), {})

async def save_partner_config(guild_id: int, data: dict):
    cfg = await load_config()
    if "partner_config" not in cfg:
        cfg["partner_config"] = {}
    cfg["partner_config"][str(guild_id)] = data
    await save_config(cfg)

async def update_partner_panel(guild: discord.Guild):
    """Aktualisiert das Partnerschafts-Panel automatisch."""
    ...
    if not partners:
        description = "Noch keine Partner vorhanden."
        view = None
    else:
        lines = []
        for p in partners:
...
async def partner_setup(interaction: discord.Interaction, kanal: discord.TextChannel):
    ...
    await interaction.followup.send(
        embed=liquid_glass_embed(
            "✅ Partnerschafts-Panel eingerichtet",
            f"Das Panel wurde in {kanal.mention} erstellt und aktualisiert sich automatisch.",
            discord.Color.from_rgb(100, 180, 255)
        ),
        ephemeral=True
    )
...
async def partner_hinzufuegen(interaction: discord.Interaction, name: str, beschreibung: str, link: str):
    if not interaction.user.guild_permissions.administrator:
        await interaction.response.send_message("❌ Keine Berechtigung!", ephemeral=True)
        return
    await interaction.response.defer(ephemeral=True)
    ...
    if any(p["name"].lower() == name.lower() for p in partners):
        await interaction.followup.send(f"❌ Ein Partner mit dem Namen **{name}** existiert bereits.", ephemeral=True)
        return
    partners.append({"name": name, "description": beschreibung, "link": link})
    ...
    await interaction.followup.send(
        embed=liquid_glass_embed(
            "✅ Partner hinzugefügt",
            f"**{name}** wurde zur Partnerliste hinzugefügt.\nDas Panel wurde automatisch aktualisiert.",
            discord.Color.from_rgb(100, 220, 150)
        ),
        ephemeral=True
    )
...
async def partner_entfernen(interaction: discord.Interaction, name: str):
    ...
    if len(new_partners) == len(partners):
        await interaction.followup.send(f"❌ Kein Partner mit dem Namen **{name}** gefunden.", ephemeral=True)
        return
    ...
    await interaction.followup.send(
        embed=liquid_glass_embed(
            "✅ Partner entfernt",
            f"**{name}** wurde aus der Partnerliste entfernt.\nDas Panel wurde automatisch aktualisiert.",
            discord.Color.from_rgb(220, 80, 80)
        ),
        ephemeral=True
    )
...
async def partner_hinzufuegen(interaction: discord.Interaction, name: str, link: str, kategorie: str = "🤝 Partner", ansprechpartner: str = None):
    ...
    if not cfg.get("channel_id"):
        await interaction.followup.send("❌ Kein Partner-Kanal konfiguriert! Führe erst `/partner-setup` aus.", ephemeral=True)
        return
    partners = cfg.get("partners", [])
    if any(p["name"].lower() == name.lower() for p in partners):
        await interaction.followup.send(f"❌ Ein Partner mit dem Namen **{name}** existiert bereits.", ephemeral=True)
    ...
    await interaction.followup.send(
        embed=liquid_glass_embed(
            "✅ Partner hinzugefügt",
            f"**{name}** wurde zur Kategorie **{kategorie}** hinzugefügt.",
            discord.Color.from_rgb(100, 220, 150)
        ),
        ephemeral=True
    )
...
@app_commands.describe(
    name="Name des Partners der bearbeitet werden soll",
