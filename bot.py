...
async def ticket_setup(
    ...
    benachrichtigungs_kanal: discord.TextChannel = None,
):
    if not interaction.user.guild_permissions.administrator:
        await interaction.response.send_message("Keine Berechtigung!", ephemeral=True)
        return
    ...
    save_ticket_config(interaction.guild.id, {
        ...
        "ping_role": ping_rolle.id if ping_rolle else None,
        "notification_channel": benachrichtigungs_kanal.id if benachrichtigungs_kanal else None,
    })
...
async def ticket_add(interaction: discord.Interaction, member: discord.Member):
    ...
    tickets = load_tickets()
    guild_id = str(interaction.guild.id)
    is_ticket = any(int(t.get("channel_id", 0)) == interaction.channel.id for t in tickets.get(guild_id, {}).values())
